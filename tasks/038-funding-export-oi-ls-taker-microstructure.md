...
- Tính trong `FundingDataCollectionManager.FundingFeatureExtractorV2` (stateful, expanding no-leak cho oiZ); OI/LS/taker đọc qua DataManager như 018 (RAM-aware, nạp 1 lần/coin rồi resample).
- APPEND vào `convertFeaturesToArray` sau `f.momentumRankCS`. N = 35 + 5 (A) + 5 (B) = ~45.
- Ghi `.bin.gz` cùng đường 037 (v3); số float/record tăng theo N. KHÔNG đè data model cũ.
