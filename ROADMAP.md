...
- `BacktestIntegrityGuard.assertProductionGrade()` cắm trong `BackTestEngineMaster.run` — tự chặn nếu chạy backtest với look-ahead/slippage/fee tắt.
...
- Trước hết **giảm gene** bằng sensitivity analysis: quét từng gene, fix cứng gene nào fitness phẳng (nghi ngờ: nhóm `AI_DYNAMIC_*`). 14 → ~8. Rồi tối ưu THEO NHÓM tuần tự (market-status → AI → DCA), khóa dần.
