...
PAT='System\.getenv\(\)?\.?(getOrDefault)?\("(SIM_|DCA_|TS_|SELECTOR_|TIER_|CONF_SIZE_|TRAIL_|GATE_|LIVE_|SIZE_MULT|MAX_CONCURRENT|TIME_STOP_HOURS|ENABLE_SHORT|ABLATION_MODE|SHORT_|SEL_BACKTEST|NUMBER_ORDER_BUDGET|HARD_STOP_LOSS_RATE|DISABLE_PREDICT_SYMBOL|CAPITAL_START|WFO_FUNDING_PRED_DIR)'
