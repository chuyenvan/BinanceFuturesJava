...
X1_LNAME=os.environ.get("X1_LNAME","cand_dev")
X1_FEAT=os.environ.get("X1_FEAT","/home/ubuntu/featv2/feat_v2.parquet")
X1_ICOUT=os.environ.get("X1_ICOUT","pool_rankic")
X1_SHUF=set(int(x) for x in os.environ.get("X1_SHUF","0,5,9").split(","))
KEEP=["vol_7d","dd_7d","rk_dd_7d","hrs_since_high_7d","ret_3d","rk_ret_3d","ret_14d","ls_global","rk_oi_delta24h","oi_z","ls_toptrader","taker_buy"]
D=pd.read_parquet(f"{LED}/{X1_LNAME}.parquet"); D=D[D.g1lite.notna()].copy(); _p("ledger nam:",D.assign(y=pd.to_datetime(D.ts,unit="ms").dt.year).y.value_counts().sort_index().to_dict()); log("pool",D.shape)
...
D["ts_h"]=(D.ts//H)*H
F=pd.read_parquet(X1_FEAT); feats_all=[c for c in F.columns if c not in ("ts","sym")]
...
_p("\n== rank-IC feat vs rel (pool) theo nam ==\n",IC.sort_values("mean",key=abs,ascending=False).round(4).to_string()); IC.to_csv(f"{LED}/{X1_ICOUT}.csv")
cut_days=os.environ.get("X1_CUTS","20220101 20220401 20220701 20221001 20230101 20230401 20230701 20231001 20240101 20240401").split()
...
def run(name,FE):
    ...
    _p("\n-- edge5 theo NAM (tat ca nam co trong OOS) --")
    for _y in sorted(set(yr.tolist())):
        _e=E[yr==_y]; _p(f"  {_y}: edge5 {100*_e.mean():+.3f}% ticks {len(_e)}")
