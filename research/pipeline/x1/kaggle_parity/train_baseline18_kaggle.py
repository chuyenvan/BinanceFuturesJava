# S1 baseline18 -- Kaggle CPU parity kernel. Self-contained (KHONG import repo).
# Copy logic tu research/analysis/s1_hpo_bag_featgrp.py::run_variant / load_D (mode baseline,
# 18-fold, KEEP9, seed 42) de do "venh" Kaggle CPU vs Oracle CPU tren CUNG mot pipeline S1.
# KHONG tune, KHONG GPU. n_jobs=4 (khop 4-CPU Kaggle CPU image, xem KAGGLE_RULES.md Sec 3c).
import glob
import json
import os
import platform
...
# --- pin xgboost==3.2.0 de khop ban Oracle (KAGGLE_RULES.md: moi trong Kaggle co the khac) ---
...
led_glob = glob.glob("/kaggle/input/**/cand_dev_x1_lite.parquet", recursive=True)
feat_glob = glob.glob("/kaggle/input/**/feat_v2_x1_keep9.parquet", recursive=True)
assert led_glob, "cand_dev_x1_lite.parquet khong tim thay trong /kaggle/input"
assert feat_glob, "feat_v2_x1_keep9.parquet khong tim thay trong /kaggle/input"
LED_PATH = led_glob[0]
FEAT_PATH = feat_glob[0]
...
print(f"=== baseline18_kaggle: edge5 all {100*E.mean():+.4f}% ticks {len(E)} "
...
P[["ts", "sym", "score", "fold"]].to_parquet(f"{WORK}/pred_baseline18_kaggle.parquet")
E.rename("edge5").to_frame().assign(fold=P.drop_duplicates("ts").set_index("ts")["fold"]).to_csv(
    f"{WORK}/edge5_by_tick_kaggle.csv")
...
with open(f"{WORK}/summary_kaggle.json", "w") as f:
