# S1 baseline18 -- Kaggle CPU determinism check (n_jobs=1). Self-contained.
# Copy nguyen train_baseline18_kaggle.py (KAGGLE_PARITY.md), CHI doi n_jobs=4 -> n_jobs=1
# va ten file output (_n1 suffix) de kiem gia thuyet determinism (PREREG_S1_DETERMINISM.md).
import glob
import json
import os
import platform
...
print(f"=== baseline18_kaggle_n1: edge5 all {100*E.mean():+.4f}% ticks {len(E)} "
...
P[["ts", "sym", "score", "fold"]].to_parquet(f"{WORK}/pred_baseline18_n1_kaggle.parquet")
E.rename("edge5").to_frame().assign(fold=P.drop_duplicates("ts").set_index("ts")["fold"]).to_csv(
    f"{WORK}/edge5_by_tick_n1_kaggle.csv")
...
with open(f"{WORK}/summary_n1_kaggle.json", "w") as f:
