...
_p("LED_PATH", LED_PATH, flush=True)
_p("FEAT_PATH", FEAT_PATH, flush=True)
_p("BASELINE_PATH (frozen, session 2026-09-19)", BASELINE_PATH, flush=True)
_p("OFI_PATH", OFI_PATH, flush=True)

t0 = time.time()
D = pd.read_parquet(LED_PATH, columns=["ts", "sym", "g1lite"])
D = D[D.g1lite.notna()].copy()
_p("pool", D.shape, flush=True)
...
del F
_p("join KEEP9: co vol_7d", D.vol_7d.notna().mean().round(3), flush=True)
...
def _read_shard(p):
    ...
    if os.path.exists(sp):
        ...
        if bad:
            _p("shard", sm.get("shard"), "bo symbol chua hoan tat:", sorted(bad), flush=True)
            x = x[~x.sym.isin(bad)]
    return x
...
_p("OFI files", len(OFI_PATHS), "rows raw", n_ofi_raw, "after exact-dedupe", len(OFI),
      "n_sym", OFI.sym.nunique(), flush=True)
D = D.merge(OFI.rename(columns={"ts": "ts_h"}), on=["ts_h", "sym"], how="left")
ofi_cov = D.ofi_1h.notna().mean()
_p("join OFI: coverage(ofi_1h notna)=", round(ofi_cov, 5),
...
_p("noise_ofi_check coverage matches ofi_1h:",
...
_p("coverage ofi_1h theo nam:", cov_by_year, flush=True)
_p("pool n_sym", int(sym_any_ofi.size), "n_sym co >=1 dong OFI", int(sym_any_ofi.sum()), flush=True)
...
def run_variant(name, FE):
    ...
    for i, c in enumerate(CUT_MS_18):
        ...
        if len(tr) < 5000 or len(oos) == 0:
            _p(name, "fold", i, "skip", flush=True)
            continue
        ...
        e = _edge5_fold(o)
        _p(f"{name} fold {i} {CUTS18[i]}: train {len(tr)} oos {len(oos)} "
...
_p("load+merge done", round(time.time() - t0, 1), "sec", flush=True)
...
_p("baseline_fresh  edge5 all", f"{100*base_fresh_e.mean():+.4f}%", "n_ticks",
      len(base_fresh_e), flush=True)
_p("baseline_frozen edge5 all", f"{100*base_frozen_e.mean():+.4f}%", "n_ticks",
...
_p(json.dumps(res_cand_vs_fresh, indent=2, default=str), flush=True)
...
_p(json.dumps(res_noise_vs_fresh, indent=2, default=str), flush=True)
...
_p(json.dumps(res_fresh_vs_frozen, indent=2, default=str), flush=True)
...
with open(f"{WORK}/ofi_result_v3.json", "w") as f:
    json.dump(result, f, indent=2, default=str)
_p("\n=== FINAL v3 ===", flush=True)
_p(json.dumps(result, indent=2, default=str), flush=True)
_p("DONE", flush=True)
sys.exit(0)
