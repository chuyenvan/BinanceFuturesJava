...
def load_bins(d):
    fs=[f for f in sorted(glob.glob(f"{d}/predict_wf_*.bin")) if os.path.basename(f)[11:19] in X1_CUTS]
    P=[]
    for f in fs:
...
if sys.argv[1]=="build":
    ...
    D.to_parquet(f"{OUT}/{X1_LNAME}.parquet"); log("saved", f"{OUT}/{X1_LNAME}.parquet", "gate_dyn_ok share:", D.gate_dyn_ok.mean().round(3))
    LOG.info("coins/tick: %s",D.groupby("ts").sym.count().describe()[["min","50%","max"]].to_dict())
else:
    D=pd.read_parquet(f"{OUT}/{X1_LNAME}.parquet"); D["yr"]=pd.to_datetime(D.ts,unit="ms").dt.year
