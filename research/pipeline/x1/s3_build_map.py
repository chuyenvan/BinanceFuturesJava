# S3 — ban SAO cua c4_build_map.py (research/pipeline/x1/c4_build_map.py), CHI them env S1_SC
#   cho DUONG DAN ledger (mac dinh = hanh vi cu: /home/ubuntu/ledger/pred_<name>.parquet).
#   Ly do: kernel Kaggle khong co /home/ubuntu/ledger. KHONG doi logic map.
# Xem docs/prereg/PREREG_STAGE3_SIM.md muc 1.1.
...
usage: s3_build_map.py <name> <out_dir>   name in {vol7d, s1a, s1b, s1a2x1}. Tick khong co score moi -> giu G015 nguyen (ghi ti le)."""
...
#     vi du: `python3 s3_build_map.py s1a2x1 $P | tail -2`.
...
# S3: SC_PATH cho phep doi duong dan ledger (mac dinh = hanh vi cu)
SC_PATH = os.environ.get("S1_SC") or f"/home/ubuntu/ledger/pred_{name}.parquet"
if name=="vol7d":
    F=pd.read_parquet("/home/ubuntu/featv2/feat_v2.parquet",columns=["ts","sym","vol_7d"]).rename(columns={"ts":"ts_h"})
    SC=None
else:
    SC=pd.read_parquet(SC_PATH)  # ts,sym,score (thap=tot) chi tren tick gate mo
    log("SC", SC_PATH, "rows", len(SC))
tot=chg=0
...
for f in sorted(glob.glob(os.environ.get("G015_BINS_DIR","/home/ubuntu/claudedata/predwf_G015x26")+"/predict_wf_*.bin")):
