...
OUT="/home/ubuntu/featv2"; NAME=os.environ.get("X1_NAME","feat_v2"); CLO="/home/ubuntu/java/fsrun/CLOSES_1H.bin"; MAP="/home/ubuntu/selector_pred_out/symbol_map.csv"
OI="/home/ubuntu/claudedata/oi/oi_percoin_full.bin"; H=3600000; T_END=int(os.environ.get("X1_T_END","1719792000000"))  # 2024-07-01 UTC (DEV cuoi)
...
# ---------- 6. long format + save ----------
# X1: dung nguyen ket qua, chi doi CACH cap phat bo nho (stack tung cot roi giai phong)
# vi P moi lon gap ~3.3 lan (43.8k gio x ~700 sym) => ban concat-tat-ca se OOM tren 23G RAM.
_stk=P.stack(dropna=False); _idx=_stk.index; _mask=_stk.notna().to_numpy(); del _stk
_cols={}
for _nm in FEATS:
    _s=F[_nm].stack(dropna=False)
    assert _s.index.equals(_idx), "index lech: "+_nm
    _cols[_nm]=_s.to_numpy()[_mask]; del _s; del F[_nm]
long=pd.DataFrame(_cols); del _cols
long.insert(0,"sym",_idx.get_level_values(1).to_numpy()[_mask])
long.insert(0,"ts",_idx.get_level_values(0).to_numpy()[_mask]); del _idx,_mask
long=long[long.ts>=1609459200000+30*86400000]                       # bo 30 ngay dau (thieu lookback)
log(f"long rows={len(long)} cols={long.shape[1]}"); long.to_parquet(f"{OUT}/{NAME}.parquet",index=False)
json.dump({"feats":FEATS,"n_rows":int(len(long)),"t_end":T_END,"src":{"closes":CLO,"oi":OI,"funding":"aerospike funding_data"}},open(f"{OUT}/{NAME}.meta.json","w"),indent=1)
