#!/usr/bin/env python3
"""make_stage2_kernel.py — dung kernel Kaggle cho STAGE 2: 6 bien the feature (V0..V5).

Khong train o day. Chi ghep 1 kernel Kaggle tu:
  (1) shim tim input + nap prefeat parquet,
  (2) toan bo `research/pipeline/g015_net_train_add.py` (base64, chay NGUYEN BAN),
  (3) buoc DO metric OOS TRONG KERNEL (rank-IC cross-section + top-8 lift theo tung tick)
      -> chi tai ve duoi dang parquet NHO (~30 MB), khong tai 5 GB bins.

ARMS doc tu `research/pipeline/featuresets/fs_v*.json` (stage2.drop_cols_arg) => KHONG go tay.
Pre-reg: docs/prereg/PREREG_STAGE2_FEATVAR.md (commit dd27c6c). 16 fold DEV (khong cham 2026).

Dung:
  python3 make_stage2_kernel.py <out_dir>
  cd <out_dir> && kaggle kernels push -p .     # (can quyen Kaggle)
"""
...
FS_DIR = os.path.join(REPO, "research", "pipeline", "featuresets")
SLUG = "g015p2-stage2-featvar-gpu"
PREF_DATASET = "chuyendinh/funding-prefeat-stage2"

# thu tu UT TIEN (neu het gio/slot thi phan dau da duoc ghi ra)
FS_FILES = [("V0", "fs_v4_21.json"), ("V5", "fs_v9_31.json"), ("V1", "fs_v5_26.json"),
            ("V2", "fs_v6_23.json"), ("V3", "fs_v7_22.json"), ("V4", "fs_v8_23.json")]
...
TEMPLATE = r'''# KERNEL: @SLUG@ -- STAGE 2: 6 bien the feature (V0..V5) tren CUNG mot lan dung ma tran.
# Pre-reg: docs/prereg/PREREG_STAGE2_FEATVAR.md (commit dd27c6c). 16 fold DEV 20220101..20251001.
# KHONG cham 2026/HoldoutSeal. KHONG cham ONNX/LIVE. KHONG sim.
import os as _os, sys as _sys, json, hashlib, base64, time, glob as _glob
import numpy as np, pandas as pd

_ROOT = "/kaggle/input"
_feat15, _labels, _oi, _map, _code, _pref = [], [], None, None, None, None
for _b, _d, _fs in _os.walk(_ROOT, followlinks=True):
    for _fn in _fs:
        _p = _os.path.join(_b, _fn)
        if _fn == "prefeat_full.parquet":
            _pref = _p
        elif _fn.startswith("features_") and (".t1c" in _fn or ".bin" in _fn):
            if "15m" in _p:
                _feat15.append(_p)
            ...
            _code = _b
assert _feat15 and _labels and _oi and _map and _code and _pref, \
    (len(_feat15), len(_labels), _oi, _map, _code, _pref)
...
_os.environ["PREFEAT"] = _pref
print("FEAT15:", len(_feat15), "| LABEL:", len(_labels), "| OI:", _oi, "| PREFEAT:", _pref, flush=True)
...
json.dump(res, open(_os.path.join(OUT, "stage2_metrics.json"), "w"), indent=1)
print("STAGE2_VERDICT " + json.dumps({"tags": TAGS, "metrics": res}), flush=True)
'''
...
META = {
    ...
    "dataset_sources": ["chuyendinh/funding-tool1-15m", "chuyendinh/funding-label-15m",
                        "chuyendinh/funding-oi-percoin", "chuyendinh/sel1m-code", PREF_DATASET],
    "kernel_sources": [],
    "competition_sources": [],
    "model_sources": [],
}
...
def build_arms():
    parts = []
    for tag, fn in FS_FILES:
        d = json.load(open(os.path.join(FS_DIR, fn)))
        dc = d["stage2"]["drop_cols_arg"]
        assert d["n_features"] == d["stage2"]["so_cot"], fn
        parts.append("%s:%s" % (tag, dc))
    return ";".join(parts)
