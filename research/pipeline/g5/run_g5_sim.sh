#!/bin/bash
# G5 SIM — ban SAO research/pipeline/x1/run_c4_sim.sh. Build dataset tu bins cua arm roi chay
# 1 sim 48 thang tren Oracle (TICKER_SOURCE=file, neo cung ho voi X1_C3), roi XOA dataset.
# usage: run_g5_sim.sh <TAG> <profile.properties> <bins_dir>
set -u
TAG=$1; PROF=$2; BINS=$3
...
DS=/home/ubuntu/wfo_ds_g5
L=/home/ubuntu/g5log
TICK=/home/ubuntu/java/simulator/kaggle_data_hpo
mkdir -p $L $B/logs

[ -n "$(pgrep java || true)" ] && { echo "G5_ABORT: JAVA_BUSY"; exit 2; }
FREE=$(df -BG --output=avail / | tail -1 | tr -dc '0-9')
echo "disk_free_G=$FREE"
[ "$FREE" -lt 8 ] && { echo "G5_ABORT: DISK_LOW ($FREE G < 8G)"; exit 3; }
grep -q "^WFO_FUNDING_PRED_DIR=$BINS\$" $PROF || { echo "G5_ABORT: profile khong tro $BINS"; exit 4; }
N=$(ls $BINS/predict_wf_*.bin 2>/dev/null | wc -l)
[ "$N" = "16" ] || { echo "G5_ABORT: co $N bins, can 16"; exit 6; }
...
[ -f $DS/market.bin ] || { echo "G5_ABORT: dataset khong sinh ra"; tail -20 $L/build_$TAG.out; exit 7; }
