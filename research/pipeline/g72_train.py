...
LAB_H = int(os.environ.get("G72_LABEL_H", "4"))
WIN = float(os.environ.get("G72_WIN", "0.06"))
H_MIN = LAB_H * 60
NEED = H_MIN // 15
C_FAV, C_NB = "maxFav_%dh" % LAB_H, "nBars_%dh" % LAB_H
NEST, SEED = 400, 42
...
def load_labels(smap):
    ...
    parts = []
    for fp in fs:
        d = FLPB.read_label(fp, usecols=["tEpochMs", "symbol", C_FAV, C_NB])
        d = d[(d[C_NB] >= NEED) & d[C_FAV].notna()]
        ...
        parts.append(pd.DataFrame({"ts": ts[keep], "symId": sid[k].to_numpy(np.int32)[keep],
                                   "y": (d[C_FAV].to_numpy(np.float64)[k][keep] >= WIN).astype(np.int8)}))
        del d
    L = pd.concat(parts, ignore_index=True)
    log.info("Label %dh WIN=%.4f: %d dong base=%.4f", LAB_H, WIN, len(L), float(L.y.mean()))
    return L
...
def main():
    t00 = time.time()
    log.info("G72 TRAIN CPU | OUT=%s NJOBS=%d NEST=%d SEED=%d LAB_H=%d WIN=%.4f",
             OUT, NJOBS, NEST, SEED, LAB_H, WIN)
