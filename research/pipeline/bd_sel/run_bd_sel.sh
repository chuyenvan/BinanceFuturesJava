...
runx() { TAG=$1; PROF=$2; D=$B/$TAG
  ...
  echo "$TAG rc=$?"
  grep -a '\[BD-SEL\]' logs/sim.out | head -3
...
runx X1_GS_T170_2021_SEL_PARITY $P/x1_gs_t170.properties
MD=$(md5sum $B/X1_GS_T170_2021_SEL_PARITY/storage/printDone.csv | awk '{print $1}')
echo "PARITY md5=$MD"
if [ "$MD" != "$REF_MD5" ]; then
...
# 2. 3 bien the
runx X1_GS_T170_2021_SEL_DROP     $P/x1_gs_t170_sel_drop.properties
runx X1_GS_T170_2021_SEL_MIX      $P/x1_gs_t170_sel_mix.properties
runx X1_GS_T170_2021_SEL_DROP_TOP8 $P/x1_gs_t170_sel_drop_top8.properties

df -BG --output=avail / | tail -1
echo "BD_SEL_DONE $(date +%T)"
