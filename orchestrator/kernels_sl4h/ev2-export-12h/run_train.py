#!/usr/bin/env python3
# EV2-EXPORT-12H — re-export EV2 predictions per-window (WFO leak-free) o HORIZON 12h thay vi 4h
#   (lever frequency #2: selector du pump o 12h co base-rate cao hon 4h -> nhieu lenh hon,
#   giai doi-tan-suat). Giu FIRST_OOS=202201 (2022 coverage) + cot oi_z/oi_delta24h/f8/f14/f22 +
#   GPU (device=cuda) + fallback CPU nguyen ven. Copy 100% preamble tu ev2-export-2022/run_train.py,
#   CHI doi label train tu maxFav_4h/nBars_4h sang maxFav_12h/nBars_12h.
# Moi window: train clfP6 (P HIT6% @12h) + clfP9 (P HIT9% @12h) tren IS (ts < cut-purge),
#   predict tren OOS [cut, cut+3m). Dump: win, ts, symbol, p6, p9, oi_z, oi_delta24h, f8, f14, f22.
...
log = logging.getLogger("ev2-export-12h")
...
def load_labels():
    # HORIZON 12h: dung maxFav_12h/nBars_12h (thay vi maxFav_4h/nBars_4h cua ev2-export-2022).
    # nBars_12h >= NEED_BARS_12H(=48, tuc 48*15m=12h) de dam bao du du lieu forward-looking.
    cols = ["tEpochMs", "symbol", "maxFav_12h", "nBars_12h"]
    df = pd.read_csv(LABEL_CSV, usecols=cols, on_bad_lines="skip").rename(columns={"tEpochMs": "ts"})
    df = df[(df["nBars_12h"] >= NEED_BARS_12H) & df["maxFav_12h"].notna()].copy()
    mf = df["maxFav_12h"].values
...
def run():
    ...
    base_rate = float(ds["hit6"].mean()) if len(ds) else 0.0
    log.info("Base rate HIT6 @12h (ca dataset): %.5f", base_rate)
    ...
    fp = os.path.join(OUT_DIR, "ev2_preds_n%d_12h.csv.gz" % N_PCT)
    allpred.to_csv(fp, index=False, compression="gzip")
    meta = {"n_pct": N_PCT, "horizon": "12h", "base_rate": base_rate,
            "rows": int(len(allpred)), "windows": int(allpred.win.nunique()),
    ...
    json.dump(meta, open(os.path.join(OUT_DIR, "ev2_export_12h_meta.json"), "w"), indent=2)
    print("EV2_EXPORT_12H_RESULT " + json.dumps(meta))
