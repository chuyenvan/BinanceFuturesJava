#!/usr/bin/env python3
# SL4H-EV2-EXPORT — dump EV2 predictions per-window (WFO leak-free) cho Java sim SL-cung.
# Moi window: train clfP6 (P HIT6%) + clfP9 (P HIT9%) tren IS (ts < cut-purge), predict tren OOS
#   [cut, cut+3m). Dump: win, ts, symbol, p6, p9. Java sim doc file nay + 1m kline that de sim path.
...
log = logging.getLogger("sl4h-ev2-export")
...
def run():
    ...
    fp = os.path.join(OUT_DIR, "ev2_preds_n%d.csv.gz" % N_PCT)
