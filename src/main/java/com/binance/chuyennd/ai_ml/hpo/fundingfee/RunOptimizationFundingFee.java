...
public class RunOptimizationFundingFee {
    ...
    private static final int POPULATION_SIZE = 30; // 2 tham số nên tăng quần thể
    ...
    public static void main(String[] args) {
        LOG.info("=== BẮT ĐẦU TỐI ƯU HÓA AI REJECT FILTER (2 PARAMS) ===");
        try {
            Configs.IS_HPO_MODE = true;
            Configs.TIME_RUN = "20260101";
            loadAndWarmUpData();
        } catch (Exception e) {
            e.printStackTrace();
            return;
    ...
    private static Float eval(Genotype<DoubleGene> gt) {
        ...
        try {
            ...
            LOG.info(String.format("Trial %4d/%d | Score: %8.1f | Trades: %4d | PnL: %6.1f$ | MaxDD: %6.1f$ | Pen: %4.1f$ | 15M: %.5f | 24H: %.5f | %s",
    ...
    private static void printFinalResult(EvolutionResult<DoubleGene, Float> result, long startTime) {
        ...
        LOG.info("\n=============================================");
        LOG.info("=== KẾT QUẢ TỐI ƯU HÓA AI REJECT FILTER (2 PARAMS) ===");
