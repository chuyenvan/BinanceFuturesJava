...
public class DataManagerAerospikeFloatSim {
    ...
    // set name 226
    public static final String AEROSPIKE_SET_NAME_FUNDING_PRED_15M = "predict_fundingfee_15m";
    ...
    public static Map<String, TreeMap<Long, Float>> getAllFundingMap() {
        // 1. Dùng ConcurrentHashMap để Thread-Safe khi nhiều luồng cùng Put
        Map<String, TreeMap<Long, Float>> allResults = new ConcurrentHashMap<>();

        // 2. Queue Thread-safe chứa các task bóc tách dữ liệu
        java.util.Queue<java.util.concurrent.Future<?>> futures = new java.util.concurrent.ConcurrentLinkedQueue<>();
        ...
        try {
            ...
            // TỐI ƯU: Đưa TypeToken ra ngoài vòng lặp để tránh khởi tạo lại nhiều lần
            final java.lang.reflect.Type mapType = new com.google.gson.reflect.TypeToken<Map<String, Float>>() {}.getType();
            ...
            getClient242().scanAll(scanPolicy, Configs.AEROSPIKE_NAMESPACE, AEROSPIKE_SET_NAME_FUNDINGFEE, (key, record) -> {
                ...
                // Copy data nguyên thủy ra biến final để ném vào luồng khác
                final byte[] compressedData = (byte[]) record.getValue("f_data");
                final Map<?, ?> fMap = record.getMap("f_map");

                // 3. Đẩy việc giải nén (Snappy) và Parse JSON nặng nề cho ThreadPool xử lý song song
            ...
            // 4. Block lại và chờ tất cả các luồng Parse data hoàn tất trước khi return
            for (java.util.concurrent.Future<?> future : futures) {
                try {
                    future.get();
                } catch (Exception e) {
                    LOG.error("❌ Lỗi khi đợi Thread giải mã: {}", e.getMessage());
                }
            }
    ...
    public static void saveFundingPredictions1M(long timestamp, Map<Short, float[]> predictions) {
        ...
        try {
            ...
            Key key = new Key(Configs.AEROSPIKE_NAMESPACE, AEROSPIKE_SET_NAME_FUNDING_PRED_15M, keyString);
...
//        System.out.println(checkAndComparePriceDiff());
    ...
    public static final String AEROSPIKE_SET_NAME_AI_PRED_MARKET = "ai_pred_market_full_basket_15m";
        ...
        try {
            ...
            predictions.entrySet().parallelStream().forEach(entry -> {
                try {
                    ...
                    LOG.error("❌ Error saving Market AI Pred for timestamp {}: {}", entry.getKey(), e.getMessage());
                }
            });
        } catch (Exception e) {
            LOG.error("❌ Error in saveMarketAiPredictionsBatch: {}", e.getMessage());
        }
    }
    ...
    public static TreeMap<Long, long[]> getFundingPredictionsPrimitiveByRange(long startTime, int totalMinutes) {
        LOG.info("📥 [HPO OPTIMIZED] Đang tải AI Symbol Predictions (Set: {} from: {} records: {})...",
                AEROSPIKE_SET_NAME_FUNDING_PRED_15M, Utils.normalizeDateYYYYMMDDHHmm(startTime), totalMinutes);
            ...
            futures.add(executor.submit(() -> {
                ...
                for (int j = 0; j < chunkTimestamps.length; j += SUB_BATCH_SIZE) {
                    ...
                    for (int k = 0; k < subKeys.length; k++) {
                        subKeys[k] = new Key(Configs.AEROSPIKE_NAMESPACE, AEROSPIKE_SET_NAME_FUNDING_PRED_15M,
    ...
    /**
     * 🔥 HÀM MỚI: Lấy 1 Batch dữ liệu Market Data 15M (Tối ưu I/O)
     *
     * @param timestamps Set chứa các mốc thời gian cần lấy
     * @return Map<Long, MarketDataObject15M> kết quả
     */
    public static Map<Long, MarketDataObject15M> getMarketData15MBatch(Set<Long> timestamps) {
        Map<Long, MarketDataObject15M> resultMap = new HashMap<>();
        if (timestamps == null || timestamps.isEmpty()) return resultMap;
        ...
        try {
            ...
            for (Long ts : timestamps) {
                String keyString = fmt.format(new Date(ts));
                keys[i] = new Key(Configs.AEROSPIKE_NAMESPACE, AEROSPIKE_SET_NAME_MARKET_DATA_15M, keyString);
                tsArray[i] = ts;
                i++;
            }

            // Dùng BatchPolicy để đọc 1 lượt nhiều records từ client226
            ...
            if (records != null) {
                for (int j = 0; j < records.length; j++) {
                    ...
                    if (record != null) {
                        byte[] compressed = (byte[]) record.getValue("data");
                        if (compressed != null) {
                            MarketDataObject15M data = MarketDataObject15M.decodeMarketDataFromBinary(compressed);
                            if (data != null) {
                                resultMap.put(tsArray[j], data);
                            }
                        }
                    }
                }
            ...
            LOG.error("❌ Error reading Market Data 15M Batch: {}", e.getMessage());
        }

        return resultMap;
    }
    /**
     * HÀM ĐỌC FULL: Tải toàn bộ dữ liệu Market Data 15M từ Aerospike vào RAM
     * (Dùng trong initData của Simulator hệ 15M)
     */
    public static TreeMap<Long, MarketDataObject15M> getAllMarketData15MFromAerospike() {
        TreeMap<Long, MarketDataObject15M> results = new TreeMap<>();
        try {
            LOG.info("📥 Đang tải FULL Market Data 15M từ Aerospike (Set: {})...", AEROSPIKE_SET_NAME_MARKET_DATA_15M);
            ScanPolicy scanPolicy = new ScanPolicy();
            scanPolicy.concurrentNodes = true; // Quét song song đa luồng từ các node AS

            getClient226().scanAll(scanPolicy, Configs.AEROSPIKE_NAMESPACE, AEROSPIKE_SET_NAME_MARKET_DATA_15M, (key, record) -> {
                try {
                    Long timestamp = record.getLong("time");
                    byte[] compressed = (byte[]) record.getValue("data");

                    // Tránh dính NullPointerException nếu record rác
                    ...
                    // Bỏ qua các record bị lỗi format/decode nếu có để không văng tiến trình
                }
            }, "data", "time");

            LOG.info("✅ Đã tải xong {} records Market Data 15M từ Aerospike.", results.size());
        } catch (Exception e) {
            LOG.error("❌ Lỗi khi Scan Market Data 15M", e);
        }
        return results;
    }
    // =========================================================================
    // 🔥🔥 KAGGLE EXPORT BATCH READERS DÀNH RIÊNG CHO HỆ 15M (Bước nhảy 15 Phút) 🔥🔥
    // =========================================================================

    public static TreeMap<Long, MarketDataObject15M> getMarketData15MByRange(long startTime, int total15mBlocks) {
        LOG.info("📥 [KAGGLE EXPORT] Đang tải Market Data 15M (Set: {})...", AEROSPIKE_SET_NAME_MARKET_DATA_15M);
        TreeMap<Long, MarketDataObject15M> results = new TreeMap<>();
        long[] allTimestamps = new long[total15mBlocks];
        for (int i = 0; i < total15mBlocks; i++) allTimestamps[i] = startTime + (i * 15 * Utils.TIME_MINUTE);

        List<Future<Map<Long, MarketDataObject15M>>> futures = new ArrayList<>();
        int chunkSize = (total15mBlocks + threadCount - 1) / threadCount;
        int SUB_BATCH_SIZE = 5000;
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            futures.add(executor.submit(() -> {
                SimpleDateFormat localKeyFormat = new SimpleDateFormat("yyyyMMdd-HHmm");
                Map<Long, MarketDataObject15M> chunkResult = new HashMap<>();
                long[] chunkTimestamps = Arrays.copyOfRange(allTimestamps, startIdx, endIdx);
                ...
                for (int j = 0; j < chunkTimestamps.length; j += SUB_BATCH_SIZE) {
                    ...
                    Key[] subKeys = new Key[limit - j];
                    for (int k = 0; k < subKeys.length; k++) {
                        subKeys[k] = new Key(Configs.AEROSPIKE_NAMESPACE, AEROSPIKE_SET_NAME_MARKET_DATA_15M,
                                localKeyFormat.format(new Date(chunkTimestamps[j + k])));
                    }
                    try {
                        Record[] records = getClient226().get(batchPolicy, subKeys);
                        if (records != null) {
                            for (int r = 0; r < records.length; r++) {
                                if (records[r] != null) {
                                    ...
                                    if (compressed != null) {
                                        MarketDataObject15M data = MarketDataObject15M.decodeMarketDataFromBinary(compressed);
                                        if (data != null) chunkResult.put(chunkTimestamps[j + r], data);
                                    }
                                }
                            }
                        }
        ...
        for (Future<Map<Long, MarketDataObject15M>> f : futures) {
            try { results.putAll(f.get()); } catch (Exception e) {}
        }
        return results;
    }

    public static TreeMap<Long, AiPredictionData> getAiPredictions15MByRange(long startTime, int total15mBlocks) {
        LOG.info("📥 [KAGGLE EXPORT] Đang tải AI Pred 15M (Set: {})...", AEROSPIKE_SET_NAME_AI_PRED_MARKET);
        TreeMap<Long, AiPredictionData> results = new TreeMap<>();
        long[] allTimestamps = new long[total15mBlocks];
        for (int i = 0; i < total15mBlocks; i++) allTimestamps[i] = startTime + (i * 15 * Utils.TIME_MINUTE);
        ...
        for (Future<Map<Long, AiPredictionData>> f : futures) {
            try { results.putAll(f.get()); } catch (Exception e) {}
        }
        return results;
    }

    public static TreeMap<Long, long[]> getFundingPredictions15MByRange(long startTime, int total15mBlocks) {
        LOG.info("📥 [KAGGLE EXPORT] Đang tải Funding Pred 15M (Set: {})...", AEROSPIKE_SET_NAME_FUNDING_PRED_15M);
        TreeMap<Long, long[]> results = new TreeMap<>();
        long[] allTimestamps = new long[total15mBlocks];
        for (int i = 0; i < total15mBlocks; i++) allTimestamps[i] = startTime + (i * 15 * Utils.TIME_MINUTE);

        List<Future<Map<Long, long[]>>> futures = new ArrayList<>();
        int chunkSize = (total15mBlocks + threadCount - 1) / threadCount;
        int SUB_BATCH_SIZE = 5000;
