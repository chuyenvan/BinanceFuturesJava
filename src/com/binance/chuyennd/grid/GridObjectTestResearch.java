 ...
 * @author pc
 */
public class GridObjectTestResearch implements Serializable {
    public static final Logger LOG = LoggerFactory.getLogger(GridObjectTestResearch.class);
    ...
    public GridObjectTestResearch(String symbol, OrderSide side, Double maxPrice, Double minPrice, KlineObjectSimple tickerStart) {
        this.symbol = symbol;
        this.side = side;
    ...
    public GridObjectTestResearch(String symbol, OrderSide side, Double maxPrice, Double minPrice, KlineObjectSimple tickerStart, Double budget) {
        this.symbol = symbol;
        this.side = side;
        this.priceStartGrid = tickerStart.priceClose;
        this.maxPrice = maxPrice;
...
//        for (Double price : prices) {
    ...
    private OrderTargetInfoTest createOrderStart(String symbol, Double price, OrderSide side, KlineObjectSimple ticker) {
        OrderTargetInfoTest order = new OrderTargetInfoTest(OrderTargetStatus.REQUEST, price,
                null, quantity/10 , leverage, symbol, ticker.startTime.longValue(), ticker.startTime.longValue(), side);
        order.tickerOpen = Utils.convertKlineSimple(ticker);
        order.marketLevelChange = MarketLevelChange.GRID_TRADE;
        order.isOrderStart = true;
        return order;
    }

    private OrderTargetInfoTest createOrderModeOnlyOrder(String symbol, Double price, OrderSide side, KlineObjectSimple ticker) {
        OrderTargetInfoTest order = new OrderTargetInfoTest(OrderTargetStatus.REQUEST, price,
                null, quantity * GridConfigs.GRID_NUMBER_ORDER_ACTIVE, leverage, symbol,
                ticker.startTime.longValue(), ticker.startTime.longValue(), side);
        order.tickerOpen = Utils.convertKlineSimple(ticker);
        order.marketLevelChange = MarketLevelChange.GRID_TRADE;
        order.isOrderStart = true;
        order.status = OrderTargetStatus.POSITION_RUNNING;
...
//        LOG.info("prices: {} \npriceActive:{}", prices, pricesActive);
    ...
    public void updateAllOrderRunning(KlineObjectSimple ticker) {
...
//                    return;
//                }
//                if (counterPositionRunning() == 0 && time2OrderDone.size() > 0){
//                    closeGrid(ticker, "take profit");
//                    return;
//                }
//            }
//
...
//            LOG.info("Grid finished: {} {}", Utils.normalizeDateYYYYMMDDHHmm(ticker.startTime.longValue()), ticker.priceClose);
    ...
    private int counterPositionRunning() {
        int counter = 0;
        for (OrderTargetInfoTest order: price2Order.values()){
            if (order.status.equals(OrderTargetStatus.POSITION_RUNNING) && order.calRateLoss() < 0){
                counter++;
            }
        }
        return counter;
    ...
    private void updatePriceActive(Double priceClose) {
        if (pricesActive.size() < 2
            ...
            Integer index = null;
            for (int i = 0; i < prices.size(); i++) {
                if (side.equals(OrderSide.BUY) || side.equals(OrderSide.BOTH)) {
    ...
    private OrderTargetInfoTest createOrderNew(Double price, Double nextPrice, KlineObjectSimple ticker) {
        ...
        OrderSide sideOrder = OrderSide.SELL;
        if (price < nextPrice) {
            sideOrder = OrderSide.BUY;
        }
        orderNew = createOrder(symbol, price, sideOrder, ticker);
...
//            );
    ...
    public Double calProfitStart() {
        Double profit = 0d;
        for (OrderTargetInfoTest order : time2OrderDone.values()) {
            if (order.isOrderStart) {
                profit += order.calTp();
            }
        }
        profit += calProfitRunning();
    ...
    public void exportFile() {
        ...
        lines.add("symbol,side, time,join, end,entry,tp,sl,quantity,status,profit");
        for (OrderTargetInfoTest order : time2OrderDone.values()) {
    ...
    public void initGridOnlyAnOrder() {
        leverage = BudgetManagerSimple.getInstance().getLeverage();
        Double budgetRun = budget;
        if (budgetRun == null) {
            budgetRun = BudgetManagerSimple.getInstance().getBudgetGrid();
        ...
        OrderTargetInfoTest order = null;
        if (side.equals(OrderSide.BUY)) {
            order = createOrderModeOnlyOrder(symbol, priceStartGrid, OrderSide.BUY, tickerStart);
        } else {
            order = createOrderModeOnlyOrder(symbol, priceStartGrid, OrderSide.SELL, tickerStart);
        }
        price2Order.put(priceStartGrid, order);

        status = OrderTargetStatus.POSITION_RUNNING;
    }

    public void updateOrderUnique(KlineObjectSimple ticker) {
        OrderTargetInfoTest order = price2Order.get(priceStartGrid);
        order.lastPrice = ticker.priceClose;
        if (side.equals(OrderSide.BUY)) {
            if (ticker.priceClose > maxPrice) {
                order.priceTP = maxPrice;
                order.status = OrderTargetStatus.TAKE_PROFIT_DONE;
                status = OrderTargetStatus.FINISHED;
            }
        } else {
            if (ticker.priceClose < minPrice) {
                order.priceTP = minPrice;
                order.status = OrderTargetStatus.TAKE_PROFIT_DONE;
                status = OrderTargetStatus.FINISHED;
            }
        }
...
//            LOG.info("Grid finished: {} {}", Utils.normalizeDateYYYYMMDDHHmm(ticker.startTime.longValue()), ticker.priceClose);
            status = OrderTargetStatus.FINISHED;
            price2Order.remove(priceStartGrid);
            time2OrderDone.put(-order.timeUpdate + time2OrderDone.size(), order);
            GridBudgetManager.getInstance().updatePnl(order);
        }
        endTime = ticker.startTime.longValue();
