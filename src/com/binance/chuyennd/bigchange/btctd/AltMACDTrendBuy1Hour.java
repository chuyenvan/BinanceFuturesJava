 ...
 * @author pc
 */
public class AltMACDTrendBuy1Hour {

    public static final Logger LOG = LoggerFactory.getLogger(AltMACDTrendBuy1Hour.class);
    private static final int NUMBER_HOURS_STOP_TRADE = Configs.getInt("NUMBER_HOURS_STOP_TRADE") ;
    ...
    private void multiRsiAndRateMa() throws ParseException {
        ...
        try {
            ...
            for (int i = 0; i < 1; i++) {
                maMaxes.add(0.2 + i * 0.02);
            }
            for (Double maMax : maMaxes) {
                lines.addAll(detectBigChangeWithParamNew(maMax));
            }
            FileUtils.writeLines(new File(AltMACDTrendBuy1Hour.class.getSimpleName() + ".csv"), lines);
    ...
    private String buildLineTest(OrderTargetInfoTest order, Double rateLoss) {
        ...
        return order.symbol + "," + Utils.normalizeDateYYYYMMDDHHmm(order.timeStart) + "," + Utils.normalizeDateYYYYMMDDHHmm(order.timeUpdate)
                ...
                + (order.timeUpdate - order.timeStart) / Utils.TIME_MINUTE + "," + maStatus;
    }
...
//                                    if (MACDTradingController.isMacdStopTrendBuy(tickers, j, orderTrade.timeStart)) {
...
//                                    }
    ...
    public static void main(String[] args) throws ParseException {
        new AltMACDTrendBuy1Hour().multiRsiAndRateMa();
