...
public class SimpleMovingAverage1HourManager {
    public static final Logger LOG = LoggerFactory.getLogger(SimpleMovingAverage1HourManager.class);
    private ConcurrentHashMap<String, TreeMap<Long, IndicatorEntry>> symbol2MaDetails;
    private static volatile SimpleMovingAverage1HourManager INSTANCE = null;
    public static String TIME_RUN = Configs.getString("TIME_RUN");
    private static String FILE_DATA_STORAGE_MA_DETAIL = "storage/simple-moving-average-detail-1h-environment.data";


    public static SimpleMovingAverage1HourManager getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new SimpleMovingAverage1HourManager();
    ...
    private void startScheduleUpdateMA() {
        new Thread(() -> {
            ...
            while (true) {
                try {
                    Thread.sleep(Utils.TIME_MINUTE * 5);
                    updateForAllSymbol();
                } catch (Exception e) {
    ...
    private void updateForASymbol(String symbol, TreeMap<Long, IndicatorEntry> date2MaDetails, Long timeUpdate) {
        timeUpdate = timeUpdate - 100 * Utils.TIME_HOUR;
        if (timeUpdate < 0) {
            timeUpdate = 0l;
        }
        LOG.info("Update ma-detail for {} {}", symbol, Utils.normalizeDateYYYYMMDDHHmm(timeUpdate));
        List<KlineObjectNumber> tickers = TickerFuturesHelper.getTickerWithStartTimeFull(symbol, Constants.INTERVAL_1H, timeUpdate);
    ...
    private void initData(boolean isProduction) {
        try {
            LOG.info("Init data ma-1h with mode production: {}", isProduction);
            if (isProduction) {
        ...
        while (true) {
            ...
            try {
                ...
                for (Map.Entry<Long, Map<String, KlineObjectNumber>> entry : time2Tickers.entrySet()) {
                    ...
                    SimpleMovingAverage1HourManager.getInstance().updateWithTicker(symbol, ticker);
                    LOG.info("Update: {} {} {} {}", symbol, Utils.normalizeDateYYYYMMDDHHmm(time),
                            ticker.priceClose, SimpleMovingAverage1HourManager.getInstance().getEntry(symbol, time).getValue());
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            IndicatorEntry entry = SimpleMovingAverage1HourManager.getInstance().getEntry(symbol, startTime);
...
//            LOG.info("Get data by time: {}", Utils.normalizeDateYYYYMMDDHHmm(date));
    ...
    public MAStatus getMaStatus(long date, String symbol) {
        date = Utils.getHour(date);
        long lastDate = date - Utils.TIME_HOUR;
...
//                        return dateStatus;
    ...
    public Double getRateChangeWithDuration(String symbol, long timeCheck, int duration) {
        ...
        for (int i = 0; i < duration; i++) {
            IndicatorEntry maDetail = date2MaDetail.get(timeCheck - (i + 1) * Utils.TIME_HOUR);
            if (maDetail != null) {
...
//            System.out.println(SimpleMovingAverage1DManager.getInstance().getMaValue("ALPHAUSDT", Utils.sdfFile.parse(TIME_RUN).getTime() + Utils.TIME_DAY));
//        }
        System.out.println(SimpleMovingAverage1HourManager.getInstance().getRateChangeWithDuration("BTCUSDT",
        ...
        System.out.println(SimpleMovingAverage1HourManager.getInstance().getRateChangeWithDuration("BLZUSDT",
