 ...
 * @author pc
 */
public class GridClientSingleton {

    public static final Logger LOG = LoggerFactory.getLogger(GridClientSingleton.class);
    ...
    private static volatile GridClientSingleton INSTANCE = null;

    public static GridClientSingleton getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new GridClientSingleton();
            INSTANCE.initClient();
        }
        return INSTANCE;
    }
    ...
    public void initClient() {
        ...
        syncRequestClient = SyncRequestClient.create(PrivateConfig.GRID_API_KEY, PrivateConfig.GRID_SECRET_KEY,
                options);
        for (ExchangeInfoEntry symbol : GridClientSingleton.getInstance().syncRequestClient.getExchangeInformation().getSymbols()) {
...
//        System.out.println(ClientSingleton.getInstance().getBalanceAvalible());
        System.out.println(GridClientSingleton.getInstance().getRateBalanceAvalible());
    }

    public double getBalance() {
        List<AccountBalance> balanceInfos = GridClientSingleton.getInstance().syncRequestClient.getBalance();
