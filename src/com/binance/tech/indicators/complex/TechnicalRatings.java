package com.binance.tech.indicators.complex;


import com.binance.tech.indicators.ma.ExponentialMovingAverage;
import com.binance.tech.indicators.ma.HullMovingAverage;
import com.binance.tech.indicators.ma.SimpleMovingAverage;
import com.binance.tech.indicators.ma.VWMovingAverage;
import com.binance.tech.indicators.misc.Ichimoku;
import com.binance.tech.indicators.oscillator.*;
import com.binance.tech.model.indicators.IchimokuEntry;
import com.binance.tech.model.*;
import com.binance.tech.model.indicators.IndicatorEntry;
import com.binance.tech.model.oscillator.*;
...
public class TechnicalRatings
{
	...
	public TechnicalRatings withStochastic(int periods, int smoothK, int smoothD)
	{
		...
		if (last(stochEntries).getK() < 20 && last(stochEntries).getD() < 20
				&& last(stochEntries).getK() > last(stochEntries).getD()
				&& prev1(stochEntries).getK() < prev1(stochEntries).getD())
			stochStatus = 1;
		else if (last(stochEntries).getK() > 80 && last(stochEntries).getD() > 80
				&& last(stochEntries).getK() < last(stochEntries).getD()
				&& prev1(stochEntries).getK() > prev1(stochEntries).getD())
			stochStatus = -1;
		else
