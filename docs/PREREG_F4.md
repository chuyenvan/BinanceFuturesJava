...
`docs/F3_SUPPLY.md` mục G đo được: `sd(p15)` trong tick = **0** ⇒ `p15` là **scalar toàn
thị trường mỗi tick**, không phải điểm từng coin. `spearman(npass, p15) = +0.454` vs
`spearman(npass, U) = +0.021`. Cộng với `docs/SELECTOR_LADDER_Q.md` (selector ladder chưa
...
thời gian, **2000 rep, seed 20260905**. Độ rộng CI nhân **`f = 1.21`** (`docs/COV_RESULT.md` §4,
...
Sai dự đoán nào thì ghi rõ dự đoán đó SAI trong `docs/F4_TIMING.md`.

## 9. Đầu ra

- Script: `research/analysis/f4_timing.py` (module `logging`, KHÔNG `print()`).
- Kết quả: `docs/F4_TIMING.md`.
- `docs/QUEUE.md`: thêm dòng `F4 — DONE <commit>`.
