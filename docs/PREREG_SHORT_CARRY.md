...
Trạng thái: **ĐANG CHỜ ĐO** (khi xong ⇒ `docs/RESULT_SHORT_CARRY.md`).
...
- `docs/RESULT_FUNDING_TOPK_ROTATE.md` (commit `55b8280`/kết quả): **LONG top-K funding nhỏ nhất**
...
- `docs/RESULT_FUNDING_FACTOR.md`: D1 (funding thấp nhất) net 24h **−0,1913%**, CI chứa 0.

**Nhưng phép đo này chỉ nói về phía LONG.** `docs/RESULT_FUNDING_SIGN.md` §0(5) chốt ở tầng
...
- **KHÔNG touch engine/Java**: engine hiện **không có đường SELL** (`ENABLE_SHORT` đã gỡ,
  `createOrderSELL` không còn — `docs/DESIGN_HEDGED_BOOK.md` §1a) ⇒ vòng này **CHỈ đo offline
...
| **QUY ƯỚC DẤU (KHOÁ)** | **`rate > 0` ⇒ LONG TRẢ, SHORT THU.** `f_pp = 100 × Σ rate` (%/notional) ⇒ `>0` = **long TRẢ / short THU**. Đã kiểm chứng 2 tầng độc lập ở `docs/RESULT_FUNDING_SIGN.md` (nguồn Binance nguyên bản, không đảo dấu). |
...
| File | Nội dung |
|---|---|
| `docs/PREREG_SHORT_CARRY.md` | file này (chốt trước, commit TRƯỚC khi đo) |
| `docs/RESULT_SHORT_CARRY.md` | kết quả + kết luận GO/NO-GO |
