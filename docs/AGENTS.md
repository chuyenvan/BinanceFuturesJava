...
| 029 fix concurrency+position (audit #4/5/9) | CCD #1 (giao) | 🟡 TODO | 2026-06-14 | live (test riêng) | race key phút, ForkJoinPool/batch, position lock+clear-then-fill |
| 030 fix parity+config/host (audit #10/11/12) | CCD #3 (giao) | 🟡 TODO | 2026-06-14 | live+sim | pred-null sim≠live (thống nhất theo live=reject); DIED user chốt prod=129 → CHỈ sync repo (không sửa live); getReadClient fail-fast |
