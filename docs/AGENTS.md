...
| 029 fix concurrency+position (audit #4/5/9) | CCD-024 | 🔵 DOING | 2026-06-14 | code (test riêng, không đụng Aerospike nặng) | race key phút, ForkJoinPool/batch, position lock+clear-then-fill |
| 030 fix parity+config/host (audit #10/11/12) | CCD-024 | 🔵 DOING (sau 029) | 2026-06-14 | code (test riêng) | pred-null sim≠live (thống nhất theo live=reject); DIED user chốt prod=129 → CHỈ sync repo (không sửa live); getReadClient fail-fast |
