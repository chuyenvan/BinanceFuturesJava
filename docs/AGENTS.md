...
| 017 feature gate B giá/xu-hướng + funding-breadth | CCD #1 | 🔵 DOING | 2026-06-14 | 226 (đọc)→CSV | B1-B5 + B7 code mới; nguồn kline_15m/4h(009)+funding_data(226); align gate_return; validate riêng a-e; **System.exit cuối main (#6)**; LÀM NGAY song song 015 (khác feature) |
| 018 feature gate B crowdedness OI/LS-market | — | ⏸ (sau 013) | — | — | B6 OI-market + B8 LS-market (aggregate=gate); CHỜ 013 backfill |
| 025 H1 ghép + export full dataset gate | — | ⏸ (sau 015+017+018) | — | 226 (ghép CSV) | §3: ghép label(012)+A(015)+B-now(017)+B-crowd(018) + validate chung (corr/leakage/drift/screen) + dataset versioned + fingerprint. **Input CSV gom ở 226 (kho compute); ghép tại 226 (không cần Aerospike nặng); ra gate_dataset_vN → đẩy Kaggle cho 026** |
...
| 033 deploy đợt 2 lên 242 (gộp 027/028/029/030/031) | CCD-024 | 🟣 REVIEW (build+runbook DONE `ff15492`; lấp-gap & deploy chờ 226/user) | 2026-06-14 | build + lấp-gap(226) | ✅ jar `mvn package` PASS (94.2MB, đủ 7 class; Configs chung 027+030; v9). ✅ runbook `docs/DEPLOY_242_dot2.md` (per-fix verify + ⚠️ check độ sâu 1m@242). ✅ Aggregate thêm arg `242` (đọc 1m@242 lấp-gap). ⚠️ **lấp-gap CHƯA chạy** (dev không tới 242 → TRÊN 226) + deploy/restart 2 live = USER tay. v9 invalidate HPO v8. ⚠️ 027 chưa commit (working tree CCD-audit) — build gồm 027 |
