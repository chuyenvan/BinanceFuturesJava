...
Trạng thái: **ĐANG CHỜ ĐO** (khi kết quả xong ⇒ `docs/RESULT_FUNDING_TOPK_ROTATE.md`).
...
1. **NO-GO** là kết cục nhiều khả năng nhất. Lý do:
   - Vòng trước (`docs/RESULT_FUNDING_FACTOR.md`, commit `5b548e4`) đo **đúng hướng này** ở dạng
...
3. **(c) Dẫn chiếu H2 vòng trước**: `docs/RESULT_FUNDING_FACTOR.md` §0/§5 — D1 (funding thấp nhất)
   ...
   Tiền lệ xa hơn: `docs/SURVEY_OLDCODE_SIGNALS.md` — rule cũ `FUNDING_FEE_BUY` → ML
...
- Kết quả: `docs/RESULT_FUNDING_TOPK_ROTATE.md`. Commit pre-reg + script + kết quả. **KHÔNG push.**
