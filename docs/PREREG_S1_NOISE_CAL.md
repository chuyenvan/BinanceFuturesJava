...
cập nhật nào của `docs/RESULT_S1_HPO_BAG_FEATGRP.md` dựa trên kết quả NOISE_CAL; nếu ngược lại
kết quả VÔ HIỆU. Thiết kế do MASTER chốt; agent thực thi KHÔNG được đổi ngưỡng/công thức/nhánh
quyết định sau khi thấy số.

## 0. Bối cảnh

`docs/PREREG_S1_HPO_BAG_FEATGRP.md` Sec 5 quy định: nếu đối chứng nhiễu `noise_0` (thêm vào
...
- **(a)** Nếu `n_select ∈ {1,2}` (khớp kỳ vọng ~13.8%/cột × 5 cột của biến cố hai phía 1.4823σ)
  ...
  phép viết `docs/RESULT_S1_HPO_BAG_FEATGRP.md` với phán quyết P1/P2/P3 = NULL theo CONFIRM
