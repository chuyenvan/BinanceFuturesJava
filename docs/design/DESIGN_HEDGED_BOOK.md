...
`docs/result/NBETS_RESULT.md`) de tang `n_eff` va thu hep CI cho moi phep so sanh selector
ve sau (`docs/notes/power_wall.md`).
...
- Khop voi `docs/analysis/RESEARCH_SHORT.md` muc 1.1 (2026-09-14) — tai lieu do ket luan y het.
...
- **`docs/analysis/RESEARCH_SHORT.md`** (short-only mirror, DA DONG): ket luan "KHONG dang build short
...
2. 🔴 **`marginRunning` → `throttle` → ADMISSION.** `throttle = 1 - marginRunning/(equity x U_MAX)`
   ...
   `docs/runbooks/AGENT_RUNBOOK.md` muc 4 da ghi nhan chinh xac co che nay o L1: doi exit → doi
...
- **(a) ICC / n_eff**: phan bo hedge PnL **pro-rata theo notional x thoi gian** ve tung lenh long
  ...
  `docs/result/NBETS_RESULT.md` muc 264–267 (`x_i = pnl_i/margin_i`, ANOVA mot chieu, cohort = ngay vao
...
- **(e) MDE80**: chay lai cap **T170-vs-T100** duoi hedge (chon cap nay vi `docs/analysis/ANALYSIS_T170_VS_T100.md`
