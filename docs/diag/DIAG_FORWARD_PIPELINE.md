...
như rủi ro đã ghi trước ở `docs/experiment/L6_GATE_DYN_FIX.md`: *"gate siết cắt ở 4% trên cùng phân phối p15
...
Thêm mục vào `docs/plan/PLAN_SHADOW_T170_PARALLEL.md`/runbook: **`Error get position from binance!`
...
Theo `docs/experiment/L6_GATE_DYN_FIX.md` ("L4 mục 7 điểm 8, chưa đo được"): trích `market[15M:..]` + `thr`
...
`docs/runbooks/AGENT_RUNBOOK.md`:
...
**Tiêu chí xong**: `grep -n "DA TAT" docs/runbooks/AGENT_RUNBOOK.md` không còn mô tả sai trạng thái.
...
5. **Có cho phép sửa `docs/runbooks/AGENT_RUNBOOK.md` §0.8(b) + §0.9** (đang mô tả SAI trạng thái)? (B6)
