...
Ngay 2026-09-07. Tiep noi `docs/L3_DEPLOY_PREP.md`, `docs/G4_RECIPE_C4.md`, `docs/G5_VALUE_LABELS.md`.
...
| **6** | **ke toan GIAY** (khong phi, khong funding, khong slippage, khong tick-size) | 🔴 **con** — nhung `mean(margin)`/equity **DA SO duoc** (L7): sim **2.498%**/leg vs so giay **3.000%->2.085%** = khop bac do lon; sai so ke toan thi chua do. `docs/L8_SIZING_PARITY_BACKLOG.md` muc 1 |
| **11** | **so giay KHONG tai lap duoc sleeve DCA / nhieu leg** (`ShadowBookC3.open` = `Map<symbol,Pos>` + `putIfAbsent` => leg 2 bi bo im lang) | 🔴 **THEM 2026-09-11 (L7).** Hai sleeve `BIG_DOWN` + `DCA_LEVEL1` = **17,291.7 / 76,428.4 = 22.6% pnl** cua backtest. Moi so cua so giay phai doc kem dieu nay. `docs/L8_SIZING_PARITY_BACKLOG.md` muc 2 |
