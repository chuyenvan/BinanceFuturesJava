...
Pre-reg: `docs/prereg/PREREG_EXIT_HIGH_N.md` (commit **`33f0b77`**, viết TRƯỚC khi chạy).
...
Mọi sim chạy **trên Kaggle CPU kernel** (`docs/runbooks/KAGGLE_SIM.md`) — **KHÔNG** chạy Java/sim trên Oracle
...
`docs/result/RESULT_GD92_X_EXIT.md` (commit `4e263c2`) gọi chân **"(A) GD92-only"**, nhưng runner của vòng đó
...
sang đường mount Kaggle (`docs/runbooks/KAGGLE_SIM.md` §0.3); hash này **không** dùng để so parity.
...
## 5. RÀO CỨNG `docs/runbooks/RISK_APPETITE.md` — theo năm **VÀ** toàn kỳ
