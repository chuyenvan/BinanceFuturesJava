...
Doc `docs/runbooks/AGENT_RUNBOOK.md` truoc. Moi job phai co `docs/PREREG_*.md` commit TRUOC khi chay.
...
`docs/prereg/PREREG_GATESCALE.md` (1985a81) / `docs/result/RESULT_GATESCALE.md`. Code+parity 71a713f. Key `SIM_GATE_DYN_SCALE`
...
`docs/experiment/L6_GATE_DYN_FIX.md`. Bo dieu kien `SELECTOR_RANK_TOPK <= 0` o `DetectEntrySignal2TradeNormal`
...
## 🔴 FLATGATE -- do chien luoc gate PHANG (cai 242 dang chay that) tren 48 thang  [DONE / `docs/result/RESULT_FLATGATE.md`]

Pre-reg `docs/prereg/PREREG_FLATGATE.md` (`ee00475`, commit TRUOC run). Huong C cua `AUDIT_GATE_DYN_PARITY` 5.2.
...
patch `docs/experiment/L6_GATE_DYN_FIX.md`, **CHUA DEPLOY, cho user duyet**.

## 🔴 GATE-DYN PARITY -- sim CO gate tang 2, live BO -- da xac minh  [DONE / `docs/audit/AUDIT_GATE_DYN_PARITY.md`]
...
## HOLDDCA -- bo time-stop + om bag + DCA 1:1 toi da 3 leg theo nhip BIG_DOWN  [DONE / `docs/result/RESULT_HOLDDCA.md`]

Pre-reg `docs/prereg/PREREG_HOLDDCA.md` (877694c). Sim Java 48 thang, nen X1_C3_FULL_PARITY_R (eq 111,428 / n 2,266).
...
## GRAVEYARD -- om bag sau time-stop + DCA 1:1 <=2 leg thay vi cat (de xuat user)  [DONE / `docs/result/RESULT_GRAVEYARD.md`]

Pre-reg `docs/prereg/PREREG_GRAVEYARD.md` (2e031f1). Offline, 285 lenh PST time-stop X1_C3_FULL, hourly close, 108 su kien BIG_DOWN.
...
## COLLAPSE_PROBE -- feature entry-time co du doan collapse khong? (offline, khong sim)  [DONE / `docs/result/RESULT_COLLAPSE_PROBE.md`]

Pre-reg `docs/prereg/PREREG_COLLAPSE_PROBE.md` (7e1bbf6). 1,996 lenh PST, nhan collapse (time-stop & ret<=-20%) n=123. AUC walk-forward
...
## BOOKCAP -- overlay tran BOOK/VON tren X1_C3_FULL (CAP12 / CAP16 / NOT40)  [DONE / `docs/result/RESULT_BOOKCAP.md`]

Pre-reg `docs/prereg/PREREG_BOOKCAP.md` commit `7e1bbf6` (chot TRUOC khi cai code). Them 2 key qua cong `Cfg`
...
## AUDIT_GATEDYN — ro soat doc lap claim "GD92 THANG" (272d8f1/1c1ecca/41d5d76)  [DONE / `docs/audit/AUDIT_GATEDYN_GD92.md`]
...
## G015ABL -- ro soat 45 feature net015, bo dan funding-drift cluster (f23, f18/f21/f22)  [DONE / `docs/result/RESULT_G015ABL.md`]

Pre-reg `docs/prereg/PREREG_G015ABL.md`. Them Stage 0 (control, retrain 45 feature) sau khi phat
...
## 5MGRID -- predict net015 luoi 5 phut (model 15 phut) + S1 train lai 5 phut  [DONE / `docs/result/RESULT_5MGRID.md`]

Pre-reg `docs/prereg/PREREG_5MGRID.md` (cap nhat 2026-09-08 sau khi Kaggle GPU train net015 o 5
...
## K12 -- SELECTOR_RANK_TOPK 8->12 tren C3_FULL 48 thang  [DONE / `docs/result/K12_RESULT.md`]

Pre-reg `docs/prereg/PREREG_K12.md` (`bc9d066`). Cong parity K=8 PASS (khop X1_EXTEND tung so).
...
## Q00000 — L4: `build_map` chay LIVE (shadow = C3 o tang entry)  [DONE / `docs/experiment/L4_LIVE_BUILDMAP.md`]
...
## Q0000 — G5: nhan/horizon cho VALUE MODEL  [DONE / `docs/experiment/G5_VALUE_LABELS.md`]

Pre-reg `docs/prereg/PREREG_G5.md` (`f0b088b`). Thiet ke: **hieu chuan CO DINH** (multiset x26/tick),
...
## Q000 — G4: recipe net015 tai dung + C4 thang GIA TRI  [DONE / `docs/experiment/G4_RECIPE_C4.md`]

Pre-reg `docs/prereg/PREREG_C4.md` (`4da1486`). Ba viec:
- **Recipe `net015` DUNG LAI DUOC** (`docs/experiment/G015_RECIPE.md` + `research/pipeline/g015_net_train.py`).
...
## Q00 — X3 trailing cap theo RANK + pre-arm SL -50%  [DONE / `docs/experiment/X3_RANKCAP_SL50.md`]

Pre-reg `docs/prereg/PREREG_X3.md` (`e622926`). 5 run tuan tu tren Oracle, 48 thang, nen `X1_C3`.
...
## Q0 — X2 exit 48 thang  [DONE c633861 / `docs/experiment/X2_EXIT48.md`]
...
E1 da dung het quota 4/4 (`docs/prereg/PREREG_EXIT.md`) va ra phan quyet TS_H=72.
...
Chi tiet `docs/audit/D1_DATA_AUDIT.md`. Trang thai:
...
- **BLOCKER cho tang tien:** `predwf_G015x26` khong co bin 2021 (som nhat 20220101)
  va G015x26 ~~KHONG reproduce duoc~~ (**DA GO** 2026-09-06, `docs/experiment/G3_X26_RECOVERY.md` —
...
5. Giai phong dia: an toan cao ~9.5 GiB, can xac nhan ~30 GiB (`docs/audit/D1_DATA_AUDIT.md`
...
Pre-reg `576e6c2`, chi tiết `docs/result/F1_FLOW_RESULT.md`.
...
Đo offline `docs/experiment/F2_COND_EXIT_MEASURE.md` (join nhãn 100%, dự báo số lệnh bị cắt 146 vs
thực tế 148 — công cụ dùng được). Pre-reg `a4b3b05`, chi tiết `docs/result/F2_RESULT.md`.
...
**fidelity** (`Q6.4`), không phải nguồn alpha. Chi tiết `docs/experiment/F3_SUPPLY.md`, script
...
đổi precision/coverage TRƯỚC. Pre-reg `1fa042d`, chi tiết `docs/experiment/F4_TIMING.md`, script
...
Pre-reg `f931265` + `6519c22`, chi tiết `docs/experiment/G1_HORIZON.md`, script
...
Chi tiết `docs/ops/BENCH_DEVICE.md`, script `research/kaggle/bench_device/`,
...
Pre-reg `e606b76`, chi tiết `docs/experiment/W1_SWEEP.md`, script `research/analysis/w1_rates.py`.
...
-> rate + equity). Pre-reg `26a45ee`, chi tiet `docs/experiment/T1_LABEL3.md`.
...
1. 🔴 **Bay #13 — bins KHONG di qua duong Kaggle.** `WFO_FUNDING_PRED_DIR` bi tieu thu o
   ...
   `docs/runbooks/KAGGLE_SIM.md §6`. Bay #14: `/kaggle/input` khong phang.
...
Pre-reg `8aa20e3`, chi tiet `docs/experiment/T2_FULLFLOW.md`. 4 run, Oracle, 2 dataset build.
...
`docs/experiment/G3_X26_RECOVERY.md`), kem hieu chuan gate
...
Pre-reg `0058c35`, chi tiet `docs/experiment/T2B_FULLFLOW.md`. 3 sim run (parity tai dung `T2_c2b_ref`).
...
## BUGS phat hien 2026-09-05 - B1/B2/B3  [DONE `5a001e5` -> baseline moi C3, `docs/experiment/C3_BASELINE.md`]
...
lan dau do duoc tran 5 that su). Pre-reg `docs/prereg/PREREG_C3.md`, chi tiet `docs/experiment/C3_BASELINE.md`,
...
Pre-reg `docs/prereg/PREREG_X1.md` (`91d7b93`) · ket qua `docs/experiment/X1_EXTEND.md`.
...
Pre-reg `docs/prereg/PREREG_H1.md` · bao cao `docs/plan/H1_HOLDOUT_PREP.md`. **Khong mo seal, khong chay
...
- 🔴 **BLOCKER 2: bins fold 2026 lay P(win) tu dau?** fold `20260101`/`20260401` cua
  ...
  `docs/experiment/G3_X26_RECOVERY.md`, tai sinh bang `research/pipeline/g015x26_train.py`). Train moi bang
...
- ⛔ Cong P2/P3/P4/P5 **chua chay** (chan boi hai blocker). Parity dataset `SIM_END_DATE=20251231`
  chua chay.
- Lenh mo seal chinh xac + chuoi 2 arm: `docs/plan/H1_HOLDOUT_PREP.md` muc 7.
...
Khao sat day du: `docs/experiment/L1_SHADOW_C3.md`. Pre-reg run doi chung da chay: `docs/prereg/PREREG_L1.md`.
...
### Q7 cap nhat 2026-09-06 — **4 chan DA MO**, shadow DANG CHAY (`docs/experiment/L2_PORT_C3.md`)
...
## Q8 — L3: cô lập LEGACY + gói deploy 242  [DEPLOYED 07/09/2026 06:29:57 / xac minh 08/09/2026 `docs/experiment/L3_DEPLOY_VERIFY_20260908.md`]
...
**Cap nhat 08/09/2026**: da xac minh 242 dang chay dung rev6/c3_shadow (deploy tu 07/09 06:29:57, ~37h sach, 0 exception, LEGACY on dinh N=65, RSS/RAM/[MAP] p50 deu trong nguong). Chi tiet: `docs/experiment/L3_DEPLOY_VERIFY_20260908.md`. Con mo: quyet dinh co tat shadow Oracle hay khong (chua kiem tra no con chay khong).

## G3 — Khoi phuc `predwf_G015x26` (2026-09-06) — **XONG, TAI LAP DUOC**

Bao cao: `docs/experiment/G3_X26_RECOVERY.md`. Script: `research/pipeline/g015x26_train.py`.
...
Lenh (Git ssh tu Windows, viet .sh -> scp -> bash; KHONG nhung &,>,<,| trong ssh inline):
  ...
  tail -n 200000 $A/logs/full.log | grep -ac 'would-BUY'   # THEO DOI, KHONG phai canh bao = 0 (tu 07/09 day la log hop le cua so giay C3 song song that; xem docs/experiment/L3_DEPLOY_VERIFY_20260908.md muc 5.2)
  ...
  free -m | head -2
Ghi 1 dong/ngay vao docs/analysis/SHADOW_LOG.md: ngay | pid | n_jvm | N_legacy | p50_min/med/max | would-BUY | skip | loi | ledger_rows | RSS | ghi chu.
  **MTM (them 11/09/2026):** chay `tools/shadow_mtm_242.sh` TREN 242 (pipe qua ssh stdin) va dan dong tom tat + 3 lenh te nhat
  ...
  realized+unrealized (docs/analysis/SHADOW_EVAL_20260911.md, DEV_COLLAPSE_CHECK_20260911.md). CANH BAO them: net%% <= -8%% (= muc collapse-day
...
`docs/experiment/L7_LEAN_GATE.md`. Goi `/home/ubuntu/deploy_242_l7/`, jar sha256 `35ffb41c…`. **242 chua cham.**
...
`docs/experiment/L8_SIZING_PARITY_BACKLOG.md`. **Khong duoc lam chung dot voi cong parity gate.**
