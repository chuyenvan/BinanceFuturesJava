...
Phép đo **OFFLINE thuần**. Pre-reg `docs/prereg/PREREG_G1.md` commit **`f931265`**,
`docs/prereg/PREREG_G1_SIM.md` commit **`6519c22`** — cả hai chốt **trước** khi thấy bất kỳ số nào.
...
`/home/ubuntu/predwf_G015_v2/` (baseline tái lập được của G015, `docs/experiment/G015_PROVENANCE.md`).
...
CI95 percentile **đã nhân độ rộng ×1.21** (`docs/result/COV_RESULT.md`).
...
⇒ **NULL. Dừng. Không chạy sim.** `docs/prereg/PREREG_G1_SIM.md` **đã không được thực thi**:
...
(mất bản export Tool1 2021, `docs/experiment/G015X26_PROVENANCE.md §1`), là **single point of failure**:
...
1. **Window overlap — caveat bắt buộc.** Động cơ ban đầu của job này là bảng "so horizon trên
   ...
   **bị nhiễm window overlap**: ROI hiện thực hoá tới **168h** nên nhãn 72h dùng chung nhiều
   ...
   `docs/result/LABEL_ROI2_RESULT.md` mục 2 **đã bị retract vì đúng lý do này**.
...
3. **Đây là single realization của DEV.** `n_eff` tỉ lệ với **độ dài lịch sử**, không với tần
   suất lấy mẫu (`docs/experiment/F4_TIMING.md`). Lấy mẫu mịn hơn không mua thêm power.
