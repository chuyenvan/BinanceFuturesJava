...
Tien le: `docs/RESULT_TRAIL_HINGE.md` (cap phang 0.05/0.12 = NULL), `docs/RESULT_TRAIL_LADDER.md`
(bac thang L1/L2/L3 = NULL), `docs/RESULT_PEAK_CLOSE.md` (dinh CLOSE = NO-GO),
`docs/RESULT_CLOSE_BIGGAP.md` (harness offline, cap 0.20/0.35 **voi dinh CLOSE** = NULL).
...
- **CI block-72h, 2000 rep, seed `20260905`, anchor `2021-07-01`** cho hieu tung rate, o **HAI do rong**:
  ...
  `docs/AUDIT_CI_INFLATE_STANDARDIZATION.md`; round nay **1 ung vien** vs baseline => `k=1`).
  **"Ngoai CI" chi tinh khi ngoai o CA HAI do rong.** HUONG: `win%`/`mP|SM`/`mP|SL`/`meanP` tang = TOT;
  `TSloss%` giam = TOT.
- **Rang buoc cung** (`docs/RISK_APPETITE.md`): `maxDD`/nam `<= 30%`; `UW <= 200` ngay; **khong nam am**;
