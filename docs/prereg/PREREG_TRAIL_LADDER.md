...
Nguon: brief cua Uni 2026-09-23 + `docs/result/RESULT_TRAIL_HINGE.md` (tien le: doi **tran PHANG** `SIM_TS_MAX_GAP`
...
2. **Parity tren Kaggle** (khong chay Java sim tren Oracle): bundle `sim-x1-2021-bundle` + jar MOI
   ...
   Tien le da xac nhan Kaggle == Oracle byte-identical cho profile/cua so nay (`docs/runbooks/KAGGLE_SIM_48M.md`).
...
2. `inflate(3) = sqrt(2 ln 3) = 1.482304` — he so chuan hoa theo `docs/audit/AUDIT_CI_INFLATE_STANDARDIZATION.md`
...
3. Chay **tuan tu/tung dot** tren Kaggle (5 slot/account, 12h kill/kernel, **chi phi 0**). Ghi ro so kernel +
   wall-clock da dung.
4. Cham theo §4; ghi `docs/result/RESULT_TRAIL_LADDER.md`; commit (**KHONG push**); don file tam.
