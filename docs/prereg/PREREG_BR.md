...
**Sai lầm đã ghi nhận:** thiết kế thí nghiệm dựa trên giả định về ý nghĩa tham số thay vì đọc chỗ nó được dùng. Đây là lần thứ 3 trong ngày (hướng score, múi giờ, giờ là ngữ nghĩa `MAX_CONCURRENT`). Quy tắc mới: **tra `docs/ops/CONFIG_INVENTORY.md` + grep chỗ dùng TRƯỚC khi viết pre-reg**.
