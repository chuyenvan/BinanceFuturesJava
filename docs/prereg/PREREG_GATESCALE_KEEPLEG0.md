...
Sim chay **tren Kaggle CPU kernel** (`docs/runbooks/KAGGLE_SIM.md`) — **KHONG** chay Java/sim tren Oracle
...
(`docs/decisions/DECISION_SHADOW_FLATGRID_KEEPLEG0.md`, commit `2fd7357`): `DCA_GRID_WEIGHTS=1,1,1,1` +
...
- uu tien so 1: **"nhieu lenh de on dinh"** (`docs/runbooks/RISK_APPETITE.md` §7);
...
| `t170_flat_keepleg0` (**KEEPLEG0**) | **1,1,1,1 + scale 6.0**, gate 1.70 | **1,085** | **103,083** | `99e42b75cf1a2142f9cd14dc72e371ba` | Oracle `java/devrun/FG_KEEPLEG0` (`docs/result/RESULT_FLATGRID.md`, commit `e9e5965`) |
...
- KEEPLEG0 toan ky: maxDD **−11.21%** · UW **147** · conc do duoc **6.98%** · tran ly thuyet
  **18.0%** (vs T170 58.5%) · **0/3 rate ngoai CI** (`docs/result/RESULT_FLATGRID.md` §1/§3/§5).
- ⚠️ **KHONG dung `FLAT_KEEPSCALE`** (1,1,1,1 giu SCALE 19.5): tran tap trung **van 58.5%**,
  conc do duoc 16.73%, FAIL rao nam (`docs/result/RESULT_FLATGRID.md` §6). Khong lien quan vong nay.
- Khau vi MOI (`docs/runbooks/RISK_APPETITE.md` §6–§7, chot 2026-09-24): `maxDD<=40%/nam` ·
...
bang so (vd lech do ticker `file` vs `aerospike`, xem `docs/runbooks/KAGGLE_SIM.md` §0) thi ghi ro tung
...
5. `docs/result/RESULT_GATESCALE_KEEPLEG0.md` + tra loi (a)(b)(c)(d). Commit SAU (khong push). Don temp.
...
- **(d)** Ket qua nay **co KHAC** so voi ket qua tren nen **1,1,3,8** khong? (`PREREG_GATESCALE_SWEEP`
  / `docs/prereg/PREREG_GATESCALE_SWEEP.md` — neu **chua co** `RESULT_GATESCALE_SWEEP.md` thi ghi ro
