...
Chỉ chạy nếu **cổng GO/NO-GO của `docs/prereg/PREREG_G1.md` §6 PASS**. Cổng FAIL ⇒ file này không
được thực thi và `docs/experiment/G1_HORIZON.md` phải ghi rõ là đã không chạy.
...
`E[max nhiễu]` N=50 = +7.2pp, DEV đã ~125 run (`AGENT_RUNBOOK §0.3`, `docs/result/NBETS_RESULT.md`).
...
trong khi `predwf_G015x26` thì không (`docs/experiment/G015X26_PROVENANCE.md §1`). Đây là **kết luận
riêng**, viết ở mục riêng của `docs/experiment/G1_HORIZON.md`, **không được trộn** với phán quyết PRIMARY
