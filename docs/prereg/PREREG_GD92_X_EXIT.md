...
|---|---|
| **GD92** (`SIM_GATE_ROLLING_PCT=0.92` + `SIM_GATE_ROLLING_DAYS=90`) | đã **THUA T170** khi chạy một mình (`docs/result/RESULT_GD92_RECHECK.md`): **0 TỐT / 3 XẤU** (win% −4.58 · TSloss% +6.41 · meanP −1.93, cả ba ngoài CI) và **FAIL rào cứng toàn kỳ** (maxDD **−16.55%**, UW **278**) |
| **HINGE V3** (`SIM_TS_PNOPUMP_WEAK_THR` 0.29→0.17) trên nền T170 | **NULL** (0/5 rate ngoài CI) — `docs/result/RESULT_TRAIL_HINGE.md` |
| **LADDER L1** (`TS_LADDER` bậc thang) trên nền T170 | **NULL** (0/5 rate ngoài CI) — `docs/result/RESULT_TRAIL_LADDER.md` |
...
| Sim chạy | **KAGGLE CPU kernel** (`docs/runbooks/KAGGLE_SIM.md`). **KHÔNG chạy Java/sim trên Oracle** (shadow active) — Oracle chỉ `mvn -o package`. |
...
2. **CI**: paired bootstrap **block-72h**, `NREP=2000`, `SEED=20260905`, anchor 2021-07-01.
   ...
   - chuẩn hoá **`inflate(k)=sqrt(2 ln k)`** với **`k=3`** (ba chân A/B/C so T170) = `1.482304`
     — theo `docs/runbooks/RISK_APPETITE.md` §1 / `AUDIT_CI_INFLATE_STANDARDIZATION.md`.
   Một rate chỉ tính là **"ngoài CI"** khi ngoài **CẢ HAI** độ rộng (tiền lệ `traillad_score.py`).
3. **Rào cứng** `docs/runbooks/RISK_APPETITE.md` (ngưỡng MỚI: maxDD ≤ 30%/năm, UW ≤ 200 ngày,
