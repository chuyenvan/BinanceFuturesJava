...
script/kết quả (đúng `docs/runbooks/AGENT_RUNBOOK.md` luật 2; thứ tự commit ngược ⇒ kết quả **VOID**). Sau khi
...
Tiền lệ phải đối chiếu: `docs/analysis/SURVEY_OLDCODE_SIGNALS.md` ghi rule cũ `FUNDING_FEE_BUY` đã chuyển sang
...
Nếu và chỉ nếu một test GO: đề xuất ở `docs/result/RESULT_FUNDING_FACTOR.md` **kèm rủi ro + điều kiện**:
...
- Kết quả: `docs/result/RESULT_FUNDING_FACTOR.md`. Commit pre-reg + script + kết quả. **KHÔNG push.**
...
`docs/analysis/SURVEY_OLDCODE_SIGNALS.md`: `FUNDING_FEE_BUY` → ML `funding_selector` → **FAIL (WFE med 0,098)**.
