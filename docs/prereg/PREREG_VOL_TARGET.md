...
> (AGENT_RUNBOOK §0.2). Mọi số liệu kết quả nằm ở `docs/result/RESULT_VOL_TARGET.md` (viết SAU).

## 0. Lý do & khung nhiệm vụ

`docs/result/GS_WAVE1_RESULT.md` kết luận trục duy nhất còn ảnh hưởng trong sim là size/leverage,
...
### 2.5 Rào cứng theo năm (khẩu vị MỚI, `docs/runbooks/RISK_APPETITE.md`, chốt 2026-09-16/17)
...
> (`maxDD%`, `UW`, `quy_min%`) in ra vẫn đúng; PASS/FAIL trong `docs/result/RESULT_VOL_TARGET.md` được
...
5. Ghi `docs/result/RESULT_VOL_TARGET.md`, commit sau khi có đủ số.
