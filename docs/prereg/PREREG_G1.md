...
nhiều đường giá hơn nhãn 4h. `docs/result/LABEL_ROI2_RESULT.md` mục 2 đã bị retract vì đúng lý do này.
...
- Script: `research/pipeline/g72_train.py` = bản **sao nguyên văn** của
  ...
  2 lần — `docs/experiment/G015_PROVENANCE.md §0`), chỉ thêm **tham số hoá nhãn qua env**
...
- So với `/home/ubuntu/predwf_G015_v2/` (10 file bins, sha256 trong `docs/experiment/G015_PROVENANCE.md §1`).
...
- **Khối:** 72h theo lịch (`block_id = floor((ts - ts_min) / 72h)`), ~304 khối trên DEV.
  Lý do: `docs/experiment/F4_TIMING.md` — `n_eff` tỉ lệ với **số khối 72h** (độ dài lịch sử), KHÔNG với
...
- CI95 = percentile 2.5/97.5, sau đó **nhân độ rộng ×1.21** quanh điểm ước lượng
  (`docs/result/COV_RESULT.md`: `sd_boot` của họ chuỗi này hụt ~21%):
...
Chi tiết + tiêu chí PRIMARY viết ở `docs/prereg/PREREG_G1_SIM.md`, **commit trước khi chạy**.
Parity fail ⇒ dừng.
...
- Không sửa file này sau khi thấy số. Nếu phát hiện lỗi thiết kế thì ghi vào phần
  "giới hạn phép đo" của `docs/experiment/G1_HORIZON.md`, không sửa ngược pre-reg.
...
`predwf_G015x26` **không reproduce được** (mất bản export Tool1 2021 — `docs/experiment/G015X26_PROVENANCE.md §1`),
...
`docs/experiment/G1_HORIZON.md`: cổng REPRO → giai đoạn 1 + CI → cổng GO/NO-GO → (giai đoạn 2 nếu có) →
