...
phai co TRUOC commit cua ket qua (`docs/ops/PAIRED_CALIB.md` + `research/analysis/paired_test.py`
chay ra so). Neu thu tu nguoc lai => toan bo ket qua tang nay bi coi la VOID.

Ly do ton tai:

- `docs/audit/CI_REAUDIT.md` (commit `a396797`): 14/18 verdict cu la **khong phan biet duoc**. Ca 3 tru
  cot cua "S1 +7.35pp CAGR" chet (`d = +7.33pp`, CI95 `[-1.72, +15.61]`).
- `docs/prereg/PREREG_GS.md` §11.1: sd cua HIEU CAGR tren DEV la 2.57pp (doi tham so exit) den 6.34pp
  (noi gate) => can **14.3 den 87.4 nam** du lieu de thay 3pp o power 80%. Tang equity/CAGR
  **vo dung lam tieu chi quyet dinh**.
- `docs/prereg/PREREG_GS.md` §11.2 da bat buoc: finalist chi duoc tuyen bo thang neu **thang o tang tung
...
Khong sua `docs/prereg/PREREG_GS.md`, `docs/prereg/PREREG_CI.md`, `research/kaggle/gsearch/*`.
...
Voi cac cap HIEU CHUAN trong `docs/ops/PAIRED_CALIB.md` thi `N = 1` (cap da chot truoc trong file
...
- **Vi sao khong bootstrap tung lenh doc lap:** cac lenh vao trong cung mot cua so gio chia nhau
  **cung mot cu dong thi truong** — cung mot con bear day tat ca altcoin xuong cung luc. Bootstrap
  ...
  `docs/analysis/LEAK_L1_REPORT.md` da chi ra (n gia). Them nua, sizing lam cac lenh mo dong thoi phu thuoc
...
So do se duoc dien vao `docs/ops/PAIRED_CALIB.md`.
...
5. Viet ket qua vao `docs/ops/PAIRED_CALIB.md` theo dung phan loai §5, kem so power §8 va
