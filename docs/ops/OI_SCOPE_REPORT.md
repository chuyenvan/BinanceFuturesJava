...
> Nguon: `docs/ops/OI_FIX_LOG.md` (cung ngay, phep do doc lap **tren chinh file** `.bin`).
...
- **CHUA PHAN GIAI #3**: **DA PHAN GIAI** — file build tu **Vision** (do phu khop dung gioi han
  2021-12-01 cua Vision metrics, `docs/data/DATA_EXTENT_SURVEY.md` muc 1), va build **voi phep dich
...
**Do lai bang cach nao** (`docs/ops/OI_FIX_LOG.md` muc 2; script `/home/ubuntu/oifix/{v5,v6,v10}.py`):
...
> dang sach. Quy tac dung: **phat hien quy uoc theo tung (symbol, ngay)**, `docs/ops/OI_FIX_LOG.md`
...
> `docs/experiment/FEAT40_LOOKAHEAD.md`. Nguyen van ben duoi **giu lai** de thay day la **mot** lan dinh
...
- `docs/analysis/LEAK_L1_REPORT.md`: rho ~0.1675 va CI kem theo.
- `docs/audit/CI_REAUDIT.md`: #8 gia tri XEP HANG `+0.0182 [+0.0085, +0.0227]`;
  #9 gia tri GATE `+0.0238 [-0.0053, +0.0442]`.
- `docs/analysis/PHASE1_DECISION_SURFACE.md`: B5 (`dyn_thr` clamp, nguong hang 1.713%,
  `profiles/c2b.properties:15-16`), B6 (K=8 vs K=5, phep do A8).
- `docs/prereg/PREREG_CI.md`: n hieu dung / DSR / PBO tinh lai.
- `docs/prereg/PREREG_GS.md` GS wave-1 (256 diem Sobol) neu co dung score selector.
- Moi so DEV/VAL cua C2b.
- **Va chinh `docs/experiment/FEAT40_LOOKAHEAD.md`**: cau "pham vi feature 1/45" va "18.55% dong DEV"
