...
- **Khẩu vị rủi ro mới**: [`RISK_APPETITE.md`](runbooks/RISK_APPETITE.md) (maxDD<=30%, UW<=200 ngày, quý
...
- **Shadow production 242 = FLATGRID KEEPLEG0** (paper, `SHADOW_NO_PUSH=true`) — xem
  [`DECISION_SHADOW_FLATGRID_KEEPLEG0.md`](decisions/DECISION_SHADOW_FLATGRID_KEEPLEG0.md).
- **Vong 09-20 (bigdown/hedge/vol-target)**: `T170` = **ALPHA** do truc tiep (%beta=3.9%,
  |t(a)|=4.372) — xem [`ANALYSIS_BETA_DECOMP_T170.md`](analysis/ANALYSIS_BETA_DECOMP_T170.md); hedge BTC
  (Phuong an A, overlay counterfactual) = **NULL** —
  [`RESULT_HEDGE_OVERLAY_A.md`](result/RESULT_HEDGE_OVERLAY_A.md); dinh chinh **ICC=0.247 la cua C2b,
  KHONG phai T170** (T170 that = 0.0516, tran 1/ICC=19.4) — xem [`power_wall.md`](notes/power_wall.md);
  vol-target COIN/PORTFOLIO = NULL rate chat luong —
  [`RESULT_VOL_TARGET.md`](result/RESULT_VOL_TARGET.md); shadow-c3 production DOWN 09-20 06:13 (jar cu
  mang STUB key) da **KHOI PHUC** 09-20 22:06.

Mục 1 "ĐỌC ĐẦU TIÊN" bên dưới mô tả trạng thái 09-03, giữ để truy vết;
[`RUNS_DEV.md`](ops/RUNS_DEV.md) / [`ROADMAP_NOLEAK.md`](plan/ROADMAP_NOLEAK.md) /
[`QUEUE.md`](plan/QUEUE.md) đứng yên từ 09-03 / 09-02 / 09-12.

**12 RESULT gần nhất theo mtime (trích dòng phán quyết đầu file, không diễn giải):**

1. [`RESULT_SHADOW_T170_FIX.md`](result/RESULT_SHADOW_T170_FIX.md) (09-18): "Trước khi sửa, shadow
...
2. [`RESULT_D3D4_FILTER_SIM.md`](result/RESULT_D3D4_FILTER_SIM.md): "Verdict: NULL — cả 3 biến thể
   KHÔNG cải thiện chất lượng; giữ nguyên T170 (parity)."
3. [`RESULT_PUMPDUMP_OHLCV.md`](result/RESULT_PUMPDUMP_OHLCV.md): "Cả 4 detector đều NULL. Không
   detector nào đạt cả 3 tiêu chí chốt trước."
4. [`RESULT_PUMPDUMP_DETECT.md`](result/RESULT_PUMPDUMP_DETECT.md): "Cả 6 feature / 4 detector đều
   NULL. Không detector nào đạt cả 3 tiêu chí chốt trước."
5. [`RESULT_POSTPUMP_MEASURE.md`](result/RESULT_POSTPUMP_MEASURE.md): "MÔ TẢ (descriptive) — KHÔNG
   chạy sim, KHÔNG sửa `.java`, KHÔNG push" (đo tín hiệu post-pump trên n=1089 lệnh).
6. [`RESULT_S1_CORRECT_MEASURE.md`](result/RESULT_S1_CORRECT_MEASURE.md): "DESCRIPTIVE MEASUREMENT —
   sửa 2 lệch cấu trúc của `RESULT_S1_RANK_QUALITY.md`... KHÔNG phải bằng chứng alpha."
7. [`RESULT_S1_RANK_QUALITY.md`](result/RESULT_S1_RANK_QUALITY.md): "DESCRIPTIVE MEASUREMENT — lần
   ĐẦU đo trực tiếp chất lượng xếp hạng của S1... KHÔNG phải bằng chứng alpha."
8. [`RESULT_TREND_RANK_IC.md`](result/RESULT_TREND_RANK_IC.md): "DESCRIPTIVE SCREENING (không phải
   bằng chứng alpha). Chạy MỘT lần, không tune."
9. [`RESULT_BD_THRESHOLD_FRAGILITY.md`](result/RESULT_BD_THRESHOLD_FRAGILITY.md): "Edge BIG_DOWN mong
   manh theo ngưỡng. PnL BIG_DOWN dao 2x (8,323 → 16,254 USD) trong 4 biến thể ngưỡng lân cận."
10. [`RESULT_DCA_AGG_PERCOIN.md`](result/RESULT_DCA_AGG_PERCOIN.md): "Per-coin 15% là công cụ ĐÚNG —
    chặn được tập trung (17.15% → 12.51%)..." — **⚠️ PRIMARY nay đã đính chính PASS→FAIL**, xem
    banner đầu file.
11. [`RESULT_DCA_ROUND_CAP.md`](result/RESULT_DCA_ROUND_CAP.md): "NULL — giữ T170. Trần 10%/lượt
    (CAP10) KHÔNG binding..." — **⚠️ PRIMARY nay đã đính chính PASS→FAIL**, xem banner đầu file.
12. [`RESULT_SEL_BIGDOWN.md`](result/RESULT_SEL_BIGDOWN.md): "Verdict: NULL — giữ nguyên T170
    (parity)."
...
1. [AUDIT_APPLIED](audit/AUDIT_APPLIED.md) — **trang thai THAT hom nay**: 57 muc da danh gia, muc nao
...
2. [C2B_SPEC](design/C2B_SPEC.md) — dac ta cau hinh dang la ung vien freeze (C2b, DEV equity **60390**):
   gate 2 tang, selector, exit, sizing, param TRO, cach tai lap.
3. [RUNS_DEV](ops/RUNS_DEV.md) — bang moi run DEV da chay + so do duoc. **Nguon su that ve "da do gi".**
4. [ROADMAP_NOLEAK](plan/ROADMAP_NOLEAK.md) — hang doi con lai + gi da dong.
5. **GATE ENTRY** — [LEAN_GATE_AUDIT](audit/LEAN_GATE_AUDIT.md) (do 48 thang: floor CO bind 1 lan =>
   **cam** rut gon gate ve `K*symbolPred`) roi [L7_LEAN_GATE](experiment/L7_LEAN_GATE.md) (cong entry ve MOT
   class `tradecore/EntryGate`, 1 knob, sim+live dung chung, `printDone.csv` byte-identical).
   Tien than: [AUDIT_GATE_DYN_PARITY](audit/AUDIT_GATE_DYN_PARITY.md), [L6_GATE_DYN_FIX](experiment/L6_GATE_DYN_FIX.md),
   [RESULT_FLATGATE](result/RESULT_FLATGATE.md). Viec con lai: [L8_SIZING_PARITY_BACKLOG](experiment/L8_SIZING_PARITY_BACKLOG.md).

## 2. CAU HINH (he `TRADING_PROFILE`, chot 2026-09-03)

- [TRADING_CONFIG_REDESIGN](ops/TRADING_CONFIG_REDESIGN.md) — thiet ke cong cau hinh B1..B5, cai da
  xong / con lai, va cac dinh chinh ghi chep (muc 5 = M1, muc 8 = cong thuc gate).
- [CONFIG_FIELD_MAP](ops/CONFIG_FIELD_MAP.md) — ban do field sinh tu ma nguon (**khong go tay**,
  chay lai `tools/gen_config_field_map.py`).
- [CONFIG_INVENTORY](ops/CONFIG_INVENTORY.md) — kiem ke key sinh tu ma nguon (**khong go tay**,
...
- [WFO_DATAFLOW](analysis/WFO_DATAFLOW.md) — luong du lieu WFO + env cua `ExportWfoDataset`.
- [insights/WFO_ROADMAP](insights/WFO_ROADMAP.md) — hub WFO.
- [DATA_GOVERNANCE_PROTOCOL](data/DATA_GOVERNANCE_PROTOCOL.md) · [DATA_CHUNKING_STANDARD](data/DATA_CHUNKING_STANDARD.md)
...
- [KAGGLE_RULES](runbooks/KAGGLE_RULES.md) — bat buoc truoc moi Kaggle job (slot=5, 12h-kill, System.exit).

## 5. PRE-REGISTRATION (khong sua noi dung da chot — chi them phu luc dinh chinh)

- [preregistration_frame_v1_2026-08-23](prereg/preregistration_frame_v1_2026-08-23.md) — khung chung.
- [PREREG_GS](prereg/PREREG_GS.md) — grid-search wave 1 (**dang chay, KHONG SUA**).
- [PREREG_H3](prereg/PREREG_H3.md) FAIL 4/5 · [PREREG_K](prereg/PREREG_K.md) VO HIEU ·
  [PREREG_BR](prereg/PREREG_BR.md) khong cai thien · [PREREG_RND](prereg/PREREG_RND.md) PASS chua bam nut.
- [PHASE1_DECISION_SURFACE](analysis/PHASE1_DECISION_SURFACE.md) — **7 quyet dinh Pha 1 cot "QUYET" con
...
- [PHASE1_RECIPE_FROZEN_v1](analysis/PHASE1_RECIPE_FROZEN_v1.md) · [PHASE1_RECIPE_DRAFT](analysis/PHASE1_RECIPE_DRAFT.md) ·
  [PHASE1_GENE_REFERENCE](analysis/PHASE1_GENE_REFERENCE.md)
- [KAGGLE_SIM](runbooks/KAGGLE_SIM.md) — chay sim SONG SONG tren Kaggle CPU (5 slot), neo **60395**,
  API `tools/kaggle_sim.py` (`submit`/`wait`/`fetch`).
- [GS_BASELINE_NOTE](analysis/GS_BASELINE_NOTE.md) — hai diem neo **60390** (aerospike) va **60395** (file):
...
- [HANDOFF-24-8-2026-B](plan/HANDOFF-24-8-2026-B.md) — ban giao 2026-08-24.
...
- [L4_LIVE_BUILDMAP](experiment/L4_LIVE_BUILDMAP.md) — `build_map` chay LIVE: shadow = C3 o tang entry (quy uoc `symbolPred = 1 - P(win)`, cong REPLAY 3 ngay DEV).
