...
Nguon: `docs/LEAN_GATE_AUDIT.md` (`e249a06`) — audit do 48 thang truoc khi sua. Tien than:
`docs/L6_GATE_DYN_FIX.md` (bat gate dong tren live rank-mode) va `docs/AUDIT_GATE_DYN_PARITY.md`.
...
1. **GIU NGUYEN tung phep nhan.** Khong rut ve `p15 >= K*symbolPred` (`K = 0.0686720`) vi
   ...
   Chi tiet: `docs/LEAN_GATE_AUDIT.md` muc 3.
...
|---|---|---|---|
| `GateRollingThreshold` (149 dong) + 2 diem `init` + re nhanh `thres15M` | `SIM_GATE_ROLLING_PCT`, `SIM_GATE_ROLLING_DAYS` | B4/GATEDYN dong | `docs/B4_RESULT.md` muc 6 (da hoi master tu truoc), `docs/AUDIT_GATEDYN_GD92.md` |
...
| FLATGATE | `SIM_GATE_DYN_BYPASS` | thi nghiem xong | `docs/RESULT_FLATGATE.md` |
...
| sizing / sleeve DCA cua so giay | **khong cham** — co y | muc 5 + `docs/L8_SIZING_PARITY_BACKLOG.md` |
...
code** => tach sang `docs/L8_SIZING_PARITY_BACKLOG.md` muc 2, dung quyet dinh cua master
("khong chung minh duoc bang test thi tach").

## 5. Sizing — **DO, khong sua** (ket qua do nam trong L8)

Master chot khong sua sizing trong L7. Da do va ghi vao `docs/L8_SIZING_PARITY_BACKLOG.md`:
...
| **LIVE** so giay C3 (8 leg mot tick, `PAPER_EQUITY=35,000`) | **3.000% -> 2.085%** | `docs/L2_PORT_C3.md` muc 4.1 (do truc tiep tu log) |

**Hai ket luan, mot trong do la dinh chinh chinh minh:**
1. Bac do lon **KHOP** (~1.0-1.2x). Uoc luong **"~16x" o `docs/LEAN_GATE_AUDIT.md` muc 4.3 la SAI**
