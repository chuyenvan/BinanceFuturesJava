...
script/kết quả (đúng `docs/AGENT_RUNBOOK.md` luật 2). Sau khi chạy **KHÔNG sửa thiết kế** (trigger,
...
Toàn bộ dưới đây **chép nguyên** từ `docs/PREREG_REVERSAL_BOUNCE.md` §2/§3/§5 và
`docs/PREREG_BIGUP_MEDIUPDOWN.md` §2/§4 (hai vòng đã chạy):
...
2. Dạng **market-level** đã được repo thiết lập = `SMALL_DOWN_15M` = **`rateDown15MAvg < -0.028`**,
   gọi là "= MOM15 live" (`docs/SURVEY_OLDCODE_SIGNALS.md` §1–§2 bảng tín hiệu;
...
|---|---|---|---|
| reversal-bounce long | 1 179 302 | **−0.080%** | `docs/RESULT_REVERSAL_BOUNCE.md` |
| BIG_UP (code cũ) | ~89 (tổng theo năm) | **+0.87%** | `docs/RESULT_BIGUP_MEDIUPDOWN.md` §3.1–3.2 |
...
- `rateDown15MAvg < -0.028` phải chiếm **≈ 0.50% số phút** (số đã in ở
  `docs/RESULT_BIGUP_MEDIUPDOWN.md` §2) và phân bố `rateDownAvg/rateDown15MAvg` phải khớp lại
...
- Script: `research/analysis/harness_control.py` (sinh tín hiệu + placebo) và
  ...
  (dọn sau khi chốt số).
- Kết quả: `docs/RESULT_HARNESS_CONTROL.md` (bảng + số + kết luận A/B/C + MDE %/lệnh).
