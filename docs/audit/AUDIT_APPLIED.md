...
> **CAP NHAT 2026-09-03 - DOC CUNG `docs/audit/CI_REAUDIT.md`.** Moi verdict trong Bang 1 duoc chot
...
> 15.44M dong / 911 ngay). Phuong phap chot truoc o `docs/prereg/PREREG_CI.md` (commit `2493eca`).
...
| C10 | H6 exit theo vol (k x vol_7d) | — | **chua chay** | — | KHONG | — | Con trong hang doi `docs/plan/ROADMAP_NOLEAK.md` (H6). Chi phi: code moi + pre-reg |
...
| C13 | `MAX_CONCURRENT` (tran lenh dong thoi) | 2026-09-03 | `K1_conc25` b:59580 va `K2_conc20` b:59580 **giong het K0 tung byte** | **VO HIEU** — nut tro khi BREAKER_MODE=OFF | **VOID** | `docs/prereg/PREREG_K.md`; `docs/ops/RUNS_DEV.md` dong K1/K2. Field con o `5f40a90^:Configs.java:230`, **da xoa o HEAD** | Sim khong he co tran cung lenh dong thoi (chi "mot lenh moi coin" + throttle von) |
...
| E8 | B4 (85 field mutable -> final) / B5 (CONFIG_HASH + PROFILE_HASH vao header printDone + manifest) | — | **chua lam** | — | **KHONG** | `docs/ops/TRADING_CONFIG_REDESIGN.md:214-220` ("Con lai (B4/B5)") | B5 la dieu kien de moi ket qua tu mang theo cau hinh sinh ra no. Chi phi thap. **Xem Bang 2 #4** |
...
| F7 | Pha 2 CPCV gate market-return tren VAL | (truoc 2026-09-02) | FAIL DSR (DSR 0.936@400 / 0.912@1400 vs nguong 0.95), 4 lan cham VAL | **FAIL, dong** | KHONG | `docs/plan/ROADMAP_NOLEAK.md:83`; memory `cpcv_*.md` (11 file archive) | Dong |
...
| F10 | H4 features spike-15m / H5 label lien tuc co do ben | — | chua chay | — | KHONG | `docs/plan/ROADMAP_NOLEAK.md` hang doi muc 4 | 9 feature hien tai deu **cham 3-14 ngay**. Day la huong ma MOI phep do 2026-09-03 deu chi ve (nut that = FEATURE). Chi phi cao (du lieu moi) |
...
- (d) **Gate van chay bang G015.** Quantile-map giu **nguyen** phan phoi P(win) cua G015 trong tung
  ...
  DINH CHINH** (`docs/analysis/LEAK_L1_REPORT.md`): **KHONG** phai "label SL 0.03 chua chot"
  ...
  cong voi **CI bi hep gia** o moi phep so dua tren pool nay — xem `docs/audit/CI_REAUDIT.md`.
...
- Repo cung noi vay: `docs/ops/TRADING_CONFIG_REDESIGN.md:58-59` va `:222`.
...
## M7 — `docs/ops/CONFIG_FIELD_MAP.md` tro `docs/design/C2B_SPEC.md`, file khong ton tai

- `CONFIG_FIELD_MAP.md` (sinh `4dd3b04`) viet: "Xem `docs/design/C2B_SPEC.md` muc 8".
- `ls docs/design/C2B_SPEC.md` -> **No such file or directory**.

## M8 — `docs/ops/CONFIG_INVENTORY.md` da cu hon code
...
## M9 — `docs/prereg/PREREG_RND.md` noi "khong mat gi", so do noi mat 0.4-0.5pp CAGR
...
- `docs/plan/ROADMAP_NOLEAK.md` bang ung vien freeze ghi "base G015 cung exit: CAGR **17.13** /
...
- `docs/analysis/PHASE1_DECISION_SURFACE.md`: cot "QUYET (1.2)" **trong hoan toan** o ca 3 bang A/B/C.
...
- => Trong khi moi cong suc 2026-09-02/03 do vao selector S1 va exit, **model cap score cho gate
  ...
  **CI bi hep gia o moi phep so** (`docs/audit/CI_REAUDIT.md`) — **khong** phai leak L1. Day van la
...
| ID | Da lam | O dau |
|---|---|---|
| **M1** | Viet lai `docs/ops/TRADING_CONFIG_REDESIGN.md` muc 5 thanh "**DA BI XOA KHOI CODE**": ghi ro commit `5f40a90` (2026-09-03 15:47 +0700, 50 file, -3582 dong) xoa ca `CONF_SIZE_*`, `SIZE_MULT`, `MAX_CONCURRENT` + 2 file test; muon dung lai **phai viet lai code**. Kem bang trang thai tung don bay (EV + ha tang). Sua ca dong "Chua dung: ..." o muc "Con lai (B4/B5)" | `docs/ops/TRADING_CONFIG_REDESIGN.md` |
| **M2** | `docs/plan/ROADMAP.md` doi dau ✅ -> ❌ cho circuit breaker, ghi ro co che DA BI XOA khoi SIM (`Configs.java:509-512` chi ho tro `OFF`), BR1/BR2/BR3 khong the chay lai, **va** phan CON DUNG: breaker VAN bat tren duong LIVE (kill-switch `637513c`). `docs/design/C2B_SPEC.md` muc 8 sua dong `BREAKER_MODE=OFF` | `docs/plan/ROADMAP.md`, `docs/design/C2B_SPEC.md` |
| **M3** | `docs/design/C2B_SPEC.md` muc 8: dong `ENABLE_SHORT=false / CONF_SIZE_MODE=0 / SIZE_MULT=1.0` sua thanh "**ba key nay KHONG CON TON TAI trong src/main**" (`5f40a90` xoa `SimulatorMarketLevelInvertedSelector.java` 555 dong + 2 test). H7 = viet code moi hoan toan | `docs/design/C2B_SPEC.md` |
| **M4** | `docs/ops/TRADING_CONFIG_REDESIGN.md` B3.4: `GateRollingThreshold` -> ghi **BAT KHA THI** (file 97 dong da xoa, `git grep` = 0 hit), khong phai "khong bat buoc"; kem so do RG95 56683 / RG97 52045 / RG95w180 59120 < C2b 60390 => EV am | `docs/ops/TRADING_CONFIG_REDESIGN.md` |
...
| **M7** | Kiem lai: `docs/design/C2B_SPEC.md` CO THAT va `## 8.` CO THAT (dong 165) => tro dan trong `CONFIG_FIELD_MAP.md:17` la **hop le**. Khong phai sua gi | — |
| **M8** | Chay lai `tools/gen_config_inventory.sh` + `tools/gen_config_field_map.py`. Inventory nay stamp `commit 19b976d` (truoc do `cb073af`, cu hon 2 commit refactor). **9 key da chet bien mat khoi kiem ke**: `CONF_SIZE_{MODE,LO,HI,FMIN,FMAX}`, `SIZE_MULT`, `MAX_CONCURRENT`, `ENABLE_SHORT`, `SIM_GATE_ROLLING_{PCT,DAYS}`. Field map: 65 -> 66 field (them `WFO_FUNDING_PRED_DIR`) | `docs/ops/CONFIG_INVENTORY.md`, `docs/ops/CONFIG_FIELD_MAP.md` |
