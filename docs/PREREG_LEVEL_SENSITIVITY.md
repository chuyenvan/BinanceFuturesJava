...
script/kết quả (đúng `docs/AGENT_RUNBOOK.md` luật 2; thứ tự commit ngược ⇒ kết quả **VOID**). Sau
khi chạy **KHÔNG sửa thiết kế** (pool, trigger, lưới 9 ô, chi phí, cổng thống kê, số rep, seed).

## 0. Vì sao + câu hỏi phải trả lời

`docs/RESULT_HARNESS_CONTROL.md` (commit `4f83902`) kết luận: bộ đo **đáng tin** (positive control
...
Chép nguyên từ `docs/PREREG_HARNESS_CONTROL.md` §2 / `PREREG_BIGUP_MEDIUPDOWN.md` §2:
...
- Kết quả: `docs/RESULT_LEVEL_SENSITIVITY.md`. File tạm dọn sau khi chốt số.
