...
- Encode vao engine: `long = (symId << 32) | Float.floatToRawIntBits(score)` voi **`score = 1.0f - pWin`**
  ...
  Engine goi gia tri nay la `symbolPred` / **`pNoPump`** (`TradeUtils.java:38-45`, `docs/AGENT_RUNBOOK.md:311`).
...
⇒ Doi `NUMBER_ORDER_BUDGET`/`CAPITAL_START` **khong doi size**. (`docs/T2B_FULLFLOW.md:31-33` cung da ghi
...
**#5 — `docs/T2B_FULLFLOW.md` mo ta cong thuc size cua BAN TRUOC FIX_B2, va so dong da lech.**
`docs/T2B_FULLFLOW.md:57-58` ghi `gridLegWeightRatio = (w[legIdx] / total) * DCA_GRID_SCALE`.
