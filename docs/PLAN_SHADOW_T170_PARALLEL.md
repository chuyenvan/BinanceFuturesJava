...
- Shadow production hiện tại (`/home/ubuntu/shadow_c3/`) đã bị đổi sang **FLATGRID
  KEEPLEG0** từ 2026-09-19 (`docs/DECISION_SHADOW_FLATGRID_KEEPLEG0.md`): DCA `1,1,1,1` /
...
🔴 **Vì sao bắt buộc port + instance Redis riêng**: `docs/L1_SHADOW_C3.md` mục 8(b) đã cảnh
...
đối chiếu game plan đã dùng khi fix shadow hiện tại — `docs/RESULT_SHADOW_T170_FIX.md`):
...
⚠️ **Bẫy đã xảy ra với instance hiện tại** (ghi trong `docs/DECISION_SHADOW_FLATGRID_KEEPLEG0.md`):
...
`docs/RESULT_SHADOW_T170_FIX.md`). Chạy lại backtest để verify env là dư thừa — hash đó đã tồn
...
2. **Đối chiếu runtime qua `/proc/<pid>/environ`** (sau khi start, xem mục 3): đúng kỹ thuật
   đã dùng ở `docs/RESULT_SHADOW_T170_FIX.md` Bước 4 — `cat /proc/<pid>/environ | tr '\0' '\n'
...
service dừng — xem mục 5.2). Đúng như `docs/RESULT_SHADOW_T170_FIX.md` đã ghi nhận: lỗi này
...
- [ ] Kill-test watchdog (giống `docs/RESULT_SHADOW_T170_FIX.md` Bước 2): `kill -9 <PID>`,
